...
Evolution seems to be an effective method for improving, therefore optimizing, behaviours. % behaviors de qué? 
To use computers for emulating the biological evolution has became an excellent tool for problem solving named in general evolutionary algorithms. %Esta intro sobra en un congreso de algoritmos evolutivos.
 For doing so the scientific community has produced several libraries of diverse quality and applied it in many domains. Nevertheless it is concentrated in the use of widespread implementation technologies like C/C++, Fortran and Java. Getting out of that mainstream it is not normally seen a land for improvement science. 
...
Other concurrent oriented languages like Ada \cite{asdsd} has been reported like a feasible technology for implemented this kind of algorithm.
