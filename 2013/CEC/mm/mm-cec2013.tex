...
% affiliations
\author{\IEEEauthorblockN{Juan-J. Merelo \\ and Antonio M. Mora}
...
\begin{abstract}
%\boldmath
There are two main research issues when in the solution to the game of
...
find fast and solutions to the game of Mastermind that scale better
...
Mastermind \cite{meirovitz1980board,Knuth,Widenius,Montgomery} is a puzzle in which one player $A$ hides a combination of
...
course, the rest of the evolutionary algorithm parameter have been
fixed by experimentation; these algorithms are shown 
...
\caption{Average proportion of search space (that is, number of evaluations divided by search space size)
  ...
  color) and the current Evo10 (solid, black). Please bear in mind
...
turns, not only for those that are above the the limit, which reflects
