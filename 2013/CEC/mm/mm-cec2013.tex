...
discoverer. 
% Antonio - en lugar de discoverer -> enquirer o algo así?
Methods for solving this puzzle 
% Antonio - Dónde? En la literatura? Usually?
are compared in terms of the number of query strings (guesses) made and the total time needed to produce those strings. In this paper we focus on the latter, by trying 
...
solution that is, at the same time, good enough. This parameter 
% Antonio - acabas de decir que era una combinación de parámetros
...
% Antonio - qué se quiere demostrar? qué se ha demostrado finalmente?
\end{abstract}
...
MasterMind \cite{Knuth,Widenius,Montgomery} 
% Antonio - con una o dos refs bastaría, no?
...
symbols in the combination that are in the correct position (usually white spots) and the number of colors that have been guessed correctly (black spots). 
% Antonio - corrígeme si me he equivocado con los colores de las pistas. :D
Player $B$ then plays new combinations until the hidden one is found. The objective of the game is to play repeatedly minimizing the number of turns needed to find the solution. 
...
computer strategy can beat a human, which uses a very different
strategy \cite{laughlin1982selection,hubalovsky2012modeling} to play it. 
% Antonio - puedes comentar en una frase qué estrategias usamos los humanos
...
% Antonio - esta expresión suena muy informal...
...
% Antonio - better es lo mismo que faster en este problema, ¿no?
...
\caption{Table of partitions after two combinations have been played;
  ...
  a game after two combinations have already been played. 
% Antonio - se repite esta misma frase al principio del caption, escríbelo de otra forma
In boldface, the combinations which have the minimal worst set size (which 
% Antonio - no has dicho lo que es el worst set size
happen to be in the 1b-1w column, 
% Antonio - no has dicho antes qué es black (b) y qué es white (w). Yo lo he aclarado en dos paréntesis, pero revísalo. ;)
but it could be any one); in this case, equal to ten. A strategy that tries to minimize worst case would play one of those combinations. The column 0b-1w with all values equal to 0 has been suppressed; column for combination 3b-1w, being impossible, is not shown either. Some rows have also been eliminated for lack of
...
\end{tabular}
\end{table*}
% Antonio - ¿por qué no pones esta tabla más cerca de donde la citas? Si se muestra antes en el artículo que el texto que explica algunas de las cosas que muestra no se entenderá...
...
{\em codebreaker} 
% Antonio - antes no has hablado de codebreaker, sino de jugador A y jugador B. De hecho, casi que mejor cambia lo de jugador A y B, ya que puede confundir con el ejemplo que pones después en el que usas 'AABB' como código. ;)
is to find the hidden combination in as few steps as
possible, she is interested in obtaining as small a partition as
% Antonio - supongo que se pone 'she' por una cuestión de discriminación positiva, pero weno, yo pondría he/she en todo caso. :p
...
required). As an example, let us consider the first combination in Table \ref{tab:mm}: 
% Antonio - supongo que te refieres a la tabla tab:partitions, la otra está comentada
...
outcome. On average, they will work as expected, 
% Antonio - si puedes usa otro término que no sea 'expected' otra vez
but even combinations with the same score will obtain a different result depending, of course, on which is the actual secret combination. Finding the best one for a particular size, 
% Antonio - ¿'the best one' qué? parece que te refieres a la 'combination'
...
\item {\em Most parts}, proposed in \cite{Kooi200513}, takes into account only the number of non-zero partitions.
...
% Antonio - alguna REF o autoREF? ahm, que vienen después, jeje.
...
% Antonio - the state of the art es qué? no será a todos los niveles... :D
...
previously published Evo method 
% Antonio - Cita que te crió
...
the consistent set size as proved in \cite{Berghman20091880,nicso}, different set sizes do have an influence on the outcome. When you reduce the
...
it. 
% Antonio - yo evitaría el 'you' en estas dos frases. ;)
...
the best score). 
However, in this paper we will prove that good solutions can be found by using a small sample size and, what is more, a common set size across all MasterMind 
% Antonio - all??? set size de qué? n osé si queda claro.
problem sizes. Using this common size means that we no longer need to worry
about  setting this parameter to find an optimal result, as we had done in previous papers \cite{DBLP:conf/cec/GuervosMC11} resulting in a great improvement on the number of experiments needed to find an acceptable solution. 
% Antonio - lo que pone aquí no cuadra con el título, fijar un parámetro no es explorar parameter space...

% Antonio - dí qué se va a estudiar con ese parámetro fijo, qué experimentos se van a hacer y qué análisis se harán. Sólo has dicho que 'good solutions can be found'... pero qué son 'good solutions'? :D

% Actualizar esto
The rest of the paper is organized as follows: the next section
(\ref{sec:evo}) 
presents the state of the resolution of the MasterMind puzzle using
evolutionary algorithms. Experiments performed and results obtained with this setup
% Antonio - qué setup?
...
solution (called {\em fitness}).
% Antonio - the optimal solution es el código, ¿no?
...
The solutions are quite promising,
...
\cite{DBLP:conf/iwann/MereloRACML11}) substituting slow parts of the
implementation for faster ones (paper submitted) 
% Antonio - ¿qué es eso de 'paper submitted'?
and uses Entropy instead of Most Parts for scoring. In general, Entropy should offer better solutions \cite{mm:evostar13}. However, the sampling size of
...
the best solutions and for which $cs$. In fact, being this one a {\em
...
% Antonio - ¿este término no se pone enlazado con guiones?
...
% Antonio - ¿experiments o runs o generations?
...
In this paper we will set this size 
% Antonio - this size -> decir lo que sea this. :D
to a common for all sizes and minimal value: 10, that is why we will denominate the method tested {\em Evo10}.
...
% Antonio - pon la tabla más cerquita, ¿no?
have been found to be the best in a previous
study (to be published).  
% Antonio - yo no diría esto de to be published. Dí cómo se han obtenido y yastá. Así parece que este tiene algo más de chicha, porque lo estoy viendo algo 'light'. ;D
...
% Antonio - ¿puedes poner alguna tablilla con los resultados de Wilcoxon? Le daría también más 'cuerpo' al trabajo. :D
...
result is significant. 
 ...
 As it can be seen in Figure \ref{fig:prop} the number of evaluations needed to find the solution (which, admittedly, is a bit worse for Evo10) 
% Antonio - yo quitaría el admittedly. Hace que el lector repare en que Evo10 es peor. Lo mismo si no lo pones, se pasa más por alto. :D
...
is in fact noticeably faster: the $\ell=6$ take on average a
second. As usual, raw data, processing scripts and computed results
...
% Antonio - ¿por qué pones las figuras y tablas tan lejos de donde las citas? :D
