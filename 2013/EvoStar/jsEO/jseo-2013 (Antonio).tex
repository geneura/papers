...
% Antonio - El título quedará escueto de más, propongo: "OO Javascript library to build cross-platform evolutionary algorithms"
...
Java-based virtual machine; peer to peer (P2P) examples also exist, as  GOLEM@Home, and G2-P2P \cite{G2-P2P}, in an attempt to avoid the bottlenecks produced by the servers.
...
Once defined the initial core set of abstract classes (except for \textit{jsEOPopulation}), concrete classes has been derived in order to implement the Standard Genetic Algorithm (\textit{jsEOGA}), that uses the tournament selector in order to create sub-populations to which mutation and crossover operators can be applied in order to make them evolve.
