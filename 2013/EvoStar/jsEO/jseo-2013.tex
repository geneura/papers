...
clients/servers ({\em servents}), usually providing their CPU cycles by means of a downloadable
application, and establishing a distributed computation network which can
provide ad hoc computational power. SETI@Home is a best well-known ALN which was quite successful \cite{david-seti:home}, as it was able to
...
As the request is being processed, it generates a series of events that can be asynchronously handled by means of the {\em callback} function, which can also access the data returned by the server.
...
in fact, this allows to control these APLs from the server with any programming
