...
The Evolutionary Computation (EC) field  is  focused on the widespread use of implementation technologies such as C/C++, Fortran and Java. Getting out of that mainstream it is not normally seen as a land for scientific improvements.
...
% Paloma: pues yo lo de "focused on the use widespread implementation technologies such as" no lo entiendo.              ¿"Focused on the widespread use of implementation technologies such as"?
...
Erlang is a programming language with a lot of potential, it supports the functional and concurrent paradigms and it is been used in the scientific community \cite{Sher2013}.
% Antonio - haz una breve inroduccióna Erlang aquí y dí si se usa o no en la comunidad científica. Pon alguna cita. ;)
% afú, qué inglés... - JJ
% Paloma - He corregido eel "it's supports" por "it supports" y el "it been use" por "it is been used".

In this paper we propose to apply this language (and its underlying paradigm) to Genetic Algorithms (GA) \cite{GA_Goldberg89},
% Antonio -  Pongo una cita a GAs
% Paloma - He incluido un "its" después del "and" entre paréntesis para que se entienda mejor.
which are general function optimizers that encode a potential solution
to a specific problem in a simple data structure (e.g. a chromosome). There
% Paloma - Propongo poner "(e.g. a chromosome)".
are only two components of them that are problem dependent: the
solution encoding and the function that evaluates the quality of a
...
% Paloma -He corregido el "evaluates" y ya está.

Many of these practices have been proposed on the object oriented paradigm
% Paloma - Nunca he visto "Many such". Yo pondría "Many of these" pero si estáis seguros de que "Many such" está bien... También he corregido la proposición, es "on", no "in".
\cite{EO:FEA2000}, but this is not the case of the functional
...
% presents considerations around the functional paradigm and its conceptual relation with evolutionary computation; we will explain then the general characteristics of a pool based concurrent evolutionary used as use case in this work. Results and conclusions obtained will be presented in Section \ref{sec:conclusions}.
...
% Antonio - he puesto GAs porque es lo qu ehas comentado en la intro. ;)
 ...
 via operators that are applied to all or a part of it. Most modern
...
The functional programming language Erlang could be a good choice since it provides the actor pattern concept for concurrency and the functional paradigm for general modeling, design and coding of solutions.
% Antonio - este texto parece de publicidad de Erlang, jeje.
% Paloma - Pero Antonio, en realidad lo que hay es que decir por qué Erlang es mejor ¿no? Pero bueno, podéis probar con "The functional programming language Erlang could be a good choice because it provides the actor pattern concept..."
%
...
also many of the typical problems of concurrent development,
% Paloma - He quitado el segundo "eliminates" para que no fuese redundante.
namely, supporting the emulation of the Object Oriented (OO) paradigm
with its modeling facilities. Functional programming, the other main feature of the Erlang language, is defined by the use of functions in program composition and by using lists.
% Antonio - Esta frase no se entiende bien y está mal escrita. ;)
% Paloma - Antonio, si no nos dices qué es lo que está mal...
...
% JA: No, no las uso por ahora, son las BDs de Erlang...
...
% JA: Había añadido "helps to modularize the code", pero ahora lo leo y creo que mejor lo quito.
...
\hline
{\tt \{generationEnd, NewIndividuals, OldIndexes, Pid\} ->}& One client reports its successfully end of calculation. \\
\hline
   \end{tabular}
...
used for solve many differents optimization problems . % Many problems qué? - JJ
 ...
 implemented in an Erlang source file and, in our case, configured in the
...
is good for applying the library to solve others practical problems with GAs using a pool based approach.  %Lo has
                                ...
                                % JA: No, a lo que me refiero es que la biblioteca está diseñada de manera que se puede usar el mismo enfoque basado en pool a otros problemas para los que habría que usar otro cromosoma, función de evaluación etc ...
...
individuals; Table \ref{resultsTable} presents the results.

\begin{table}
  \centering
   \begin{tabular}{|>{\centering\arraybackslash}m{2cm}|>{\centering\arraybackslash}m{2cm}|>{\centering\arraybackslash}m{2.5cm}|}
   \hline
   \textbf{No. clients} & \textbf{Assignments}& \textbf{Time (seconds)}\\
     \hline
    25 & 3040 & 5.339950 \\
\hline
    50 & 4301 & 7.891000 \\
...
\hline
\end{tabular}
  \caption{Experiments results.}\label{resultsTable}
\end{table}

%50 clients were used and they worked with 20 individuals
...
This results shows that in this case there it is not
...
% forma secuencial? - JJ
% JA: Si, ellos trabajan concurrente, el problema a de estar en otra parte del algoritmo... creo que ahora es muy tarde para encontrarlo.

With this concept test we are showing how simple is to structure a parallel GA, and now we could proceed with more complex GA models, experiments and problems in order to explore the potential of the technology.
