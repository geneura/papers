...
\title{Service Oriented Evolutionary Framework: applications and results}
...
\begin{abstract}
This paper shows the stage of development of a Service Oriented Architecture for Evolutionary Algorithms and the first results attained in two different areas: Parameter Adaptation and Evolutionary Art. An abstract architecture is presented, with its assocciated implementation using a wide-used technology. Results attained in experiments with parameter adaptation in distributed heterogeneous machines are presented and the usage of the architecture in Evolutionary Art is also applied.
\end{abstract}
...
%Es que OSGi no necesita una
                                %librería externa? ¿Por qué usar
                                %librerías externas tendría que ser
                                %malo?
                                %FERGU: OSGi usa una librería externa pero no necesita código específico para usarla (lo hace desde fuera). Es decir, si usas MPI tienes que meter código de MPI en mitad de tu algoritmo (MPI_send, MPI_receive...), pero con OSGi no. Eso también está explicado más adelante (aunque, he añadido "so the code to use this libraries is mixed with the algoritmh's code")
...
%%%%%%%%%%%%%%%%%%  Experiments  %%%%%%%%%%%%%%%%%%%
\section{Service Oriented Architecture for Evolutionary Algorithms}
