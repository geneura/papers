...
New trends such as Cloud Computing \cite{CLOUD}, GRID
...
Algorithms (dEAs) \cite{AQUIEN} have been tested successfully in these
...
be divided in two categories: a dEA where the parameters are different
in each node (heterogeneous parameters), or the same algorithm in
heterogeneous hardware (with the same parameters). It  has also been
...
as different memory access times, cache, % cache qué? - JJ
...
configuration  has also been proved as more efficient than a fixed
set % CUIDADO CON LA GRAMÁTICA!!! - JJ
of parameters for different problems
\cite{HETEROGENEOUSPARAMETERS}.   % efficiente en qué sentido? - JJ
