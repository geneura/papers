...
\begin{abstract}
This paper presents a study on population size adaptation in a distributed Evolutionary Algorithm. The total individuals number is divided taking into account the computational power of each node of an heterogeneous network of computers. The same population sizes are also tested in an homogeneous scientific cluster. Two problems with different characteristics have been tested. Results show that setting the population size according to the computational power decreases the time required to obtain the optimum in both problems when using heterogeneous clusters. Also, a study of the influence of the different population sizes in each stage of the algorithm is presented. This opens a new research line to adapt (offline or online) parameters to the computational power of the devices.
...
In distributed EAs (dEAs) a number of nodes executes simultaneously the EA, working with different sub-populations at the same time. Each certain number of generations is interchanged (migrated) between populations. Figure \ref{fig:islands} shows this model with a ring topology. Distributed EAs are very popular because the implementation is
...
Population size in HeSi for MMDP & 374, 364, 262, 24 \\ \hline
Population size in HeSi for OneMax & 396,  382, 232 and 14 \\ \hline
\hline\end{tabular}
...
\begin{figure}[htb]
\centering
\epsfig{file=images/figure11.eps, width = 14cm}
...
\begin{figure}[htb]
\centering
\epsfig{file=images/figure12.eps, width = 14cm}
