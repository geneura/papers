...
% Antonio - ¿Sería 'Prediction' o 'Forecasting'?
...
The development of new systems that inform of traffic conditions and the affluence of vehicles on roads, seem to be a key point nowadays. 
% muy débil esa afirmación ¿Lo parece o lo es? ¿A quien le parece?
With an increasingly informed population, provided with ubiquitous
communication devices, which are commonly used by more than $ 90 \% $
% de dónde es ese dato? Tratad de haced afirmaciones siempre sober
% algún dato publicado - JJ
of the population,   obtaining information about the traffic in any
part of the above 165000 kilometres of Spanish roads would mean an
optimal management of a communication network fundamental for a high
percentage of users. % ¿Y de dónde habéis sacado eso? Además, el
                     % experimento está hecho sobre calles, no sólo
                     % carreteras. - JJ
...
be used in a time-series forecasting process. % Deberíais hacer aquí
                                % algún "claim" sobre qué es lo que va
                                % a hacer ese sistema, sus pros y sus
                                % contras . Si os vais a concentrar en
                                % la posibilidad de predicción,
                                % decidlo así en el paper (y qué grado
                                % de acierto o error sería aceptable)
                                % - JJ

The data-collecting and the time-series forecasting processes [REF] 
% Antonio - poner una referencia y contar en una línea lo que son
...
to their vehicles, as well as mobile phones, tablets or laptops.  % decir las limitaciones que tienen estos últimos - JJ
...
% Anunciad aquí lo que vais a hacer y por qué es interesante; ¿qué
% interés tiene saber cuantos coches hay en días laborables? - JJ
...
% Asigna una tarea a alguien en GitHub para que se haga. Quizás Pedro
% García. 
...
% Esto debería ir antes del estado del arte. Así te puedes referir a
% él - JJ
% Antonio - pos eso, es una sección conjunta. a la vez que se explican los métodos se comenta el estado del arte en los mismos. ;)
Traffic detection technologies can be generally classified into two groups: intrusive and non-intrusive.

%Vamos a ver si de una vez por todas dejáis de copiar frases literales
%de "por ahí". Esta frase está sacada de http://www.ugpti.org/research/projects.php?view=123&program=atac
...
% processing). 
%También está copiado. Por favor, no copiéis nunca frases ni siquiera
...
% Antonio - hecho. Reescribo además cada punto para que no nos digan ná. ;D
...
Finally, from a privacy point of view, the Spanish Data Protection Agency\footnote{Agencia de Protección de Datos} considers the car license plate as a personal data, so their use could require the user consent.
...
%aunque sean comerciales deberíais buscar referencias donde se usen. 
...
% Antonio - hay que darle muuucho más bombo a lo que se hace en el artículo con el método que proponemos. Decir sus ventajas (bajo coste, gran capacidad de detección, etc) y compararlas con los métodos existentes.

%===============================================================================

\subsection{Time series forecasting}
% Debería ir también en la introducción, antes del estado del arte. 
...
information about foot traffic through Cordoba city center. % Esta
                                % frase ya la habéis dicho al
                                % principio. O la mudáis allí o
                                % borráis la parte "publicitaria" - JJ
...
??????DescripciÃ³n de los datos recopilados
...
devices. % Se va a liberar ese dato? Convendría hacerlo (también por
...
In this section the analysis of collected data during the monitoring period (November 8 to December 9) to obtain statistics and so study the use of vehicles is carried out.
 ...
 % informaciÃ³n acerca del nÃºmero total de vehÃ­culos detectados por
 % cada nodo, en dÃ­as laborables o festivos, sobre la densidad del
 % trÃ¡fico por rango horario, acerca de los desplazamientos
...
consecutive nodes.
% Convendría que buscárais resultados similares en otros trabajos para
% comparar y probar efectivamente que son razonables los resultados
% son correctos (y ha sido más barato obtenerlos, por ejemplo) - JJ

 % Por Ãºltimo, puesto que el dÃ­a 14 de noviembre de 2012 se celebrÃ³ la huelga general, estudiaremos cÃ³mo afectÃ³ la huelga al trÃ¡fico en el Ã¡rea metropolitana de Granada, comparando los totales de detecciÃ³n de dispositivos entre el dÃ­a de la huelga (14 de noviembre), y el miÃ©rcoles siguiente (21 de noviembre).
...
\subsection{Total number of vehicles detected (weekdays and holidays)}
 % VehiculosTotales.txt tiene los vehÃ­culos totales detectados en cada nodo.

% La primera parte del anÃ¡lisis ha consistido en calcular el nÃºmero de dispositivos detectados por cada uno de los nodos instalados.
...
Node Id.  &  N. of devices detected  \\
 ...
 % Como se observa en la Tabla \ref{VehiculosTotales}, los dos nodos situados en la autovÃ­a de Sierra Nevada (A44, CircunvalaciÃ³n de Granada, nodos 4 y 5) son los que mÃ¡s datos han recogido, mientras que el situado en una calle secundaria (nodo 6) ha sido el que menos dispositivos ha recogido.
...
\subsection{Total vehicles detected on non-working days}
 % VehiculosFestivos.txt tiene los vehÃ­culos detectados en dÃ­a festivo en cada nodo.

 % A continuaciÃ³n, y para comparar la intensidad circulatoria entre dÃ­as laborables y no laborables, se ha calculado el nÃºmero de pasos en dÃ­as festivos y no laborables.
...
Node Id.  &  N. of devices detected  \\
 ...
 % La Tabla \ref{VehiculosFestivos} muestra un descenso en el nÃºmero de dispositivos detectados por todos los nodos en dÃ­as no laborables, frente al nÃºmero de detecciones en dÃ­as laborables. 
 % AÃºn asÃ­, los nodos situados en la autovÃ­a de Sierra Nevada siguen recogiendo muchos mÃ¡s datos que el resto, debido al trÃ¡fico denso que soporta esta vÃ­a en dÃ­as no laborables.
...
\subsection{Traffic density on the road by time range}
 % VehiculosDiferentesPorHoras.txt contiene para cada uno de los nodos, los dispositivos diferentes detectados en cada rango horario (final de la lÃ­nea)
...
Traffic density can be calculated taking into account the total number of detected devices by time range.
 ...
 \end{center}
 \end{figure}
 % GrÃ¡fica (histograma) de la densidad de la vÃ­a por horas: desde las 00-7h, de 7-10h, de 10-13h, de 13-16h, de 16-20h, y de 20-24h.
...
\subsection{Total detections by time range}
 % VehiculosPorHoras.txt indica para cada nodo, el nÃºmero de dispositivos detectados en el rango horario, sin diferenciar si el dispositivo es siempre el mismo o no. 

 % Adicionalmente podemos calcular para cada nodo, el nÃºmero de dispositivos detectados en cada rango de horas, sin diferenciar si el dispositivo es siempre el mismo o no. AsÃ­ pues, se incluirÃ¡n pasos repetidos del mismo vehÃ­culo.
...
\subsection{Number of individual vehicles detections}
 % Intervalos.txt contine el nÃºmero de vehÃ­culos detectados entre 0 y 5 veces, entre 5 y 10 veces y asÃ­ sucesivamente hasta mÃ¡s de 25 veces detallado en cada nodo.

 % A continuaciÃ³n podemos sacar partido de la capacidad del sistema propuesto para individualizar los dispositivos BT, pudiendo detectar si esos mismos vehÃ­culos repiten paso por diferentes nodos.
We can take advantage of the proposed system's ability to identify BT devices. Thus, it can be detected whether vehicles pass by different nodes.
 ...
 % Para cada uno de los nodos se muestra el total de dispositivos detectados cierto nÃºmero de veces (repetidas apariciones del mismo dispositivo)
 \label{Intervalos}}
 \end{center}
 \end{figure} 
 % Histograma que muestre cuÃ¡ntos coches se detectan entre una y 5 veces, entre 6 y 10, entre 10 y 20, y mÃ¡s de 20 veces.

 % En la Figura \ref{Intervalos} podemos ver que hay una gran cantidad de vehÃ­culos que repiten su paso por ciertos nodos (principalmente los situados en la A44) hasta 10 veces. 
 % Incluso se puede ver que en los nodos 4 y 5 hay alrededor de 1000 vehÃ­culos que repiten su paso mÃ¡s de 25 veces. En el resto de nodos, mÃ¡s de 25 repeticiones del mismo dispositivo se han detectado alrededor de 120 veces solamente.
...
\subsection{Complexity of displacement}
 % NodosPorDondePasan.txt contiene el nÃºmero de vehÃ­culos que han pasado por 2 nodos, por 3 nodos y asÃ­ hasta por los 6 nodos y el nÃºmero medio de veces que han pasado 2, 3, 4, 5 o 6 veces por cada nodo.

 % En el estudio de la complejidad de los desplazamientos se han calculado el nÃºmero de vehÃ­culos que han pasado por 2 nodos, por 3 nodos y hasta 6 nodos. En la Tabla \ref{tNodosPorDondePasan} se muestra ademÃ¡s el nÃºmero medio de veces que han pasado un vehÃ­culo por 2, 3, 4, 5 o 6 nodos.
...
6 & 	61 & 	 3731 & 	$61.16 \pm 94.78$  \\
 ...
 % NÃºmero de vehÃ­culos que han pasado por 2 nodos, por 3 nodos y hasta 6 nodos, y el nÃºmero medio de veces que han pasado los vehÃ­culos por 2, 3, 4, 5 o 6 nodos. En algunos casos las desviaciones son altas debido a que algunos dispositivos tienen un nÃºmero muy alto de apariciones por algunos de los nodos
 \label{tNodosPorDondePasan}}
 \end{table}

 % La informaciÃ³n anterior se complementa visualmente con la Figura \ref{fNodosPorDondePasan}, en la que se muestra (en escala logarÃ­tmica) cuÃ¡ntos coches pasan sÃ³lo por un nodo, por dos nodos, por tres nodos, etc.
The above information is complemented with Figure \ref{fNodosPorDondePasan}, that shows how many cars pass by only one node, two nodes, three nodes, etc.
 ...
 % La figura muestra cuÃ¡ntos coches pasan sÃ³lo por un nodo, por dos nodos, por tres nodos, etc. Para mejorar la visualizaciÃ³n se ha utilizado escala logarÃ­tmica
 \label{fNodosPorDondePasan}}
 \end{center}
 \end{figure}
 % Histograma que muestre cuÃ¡ntos coches pasan sÃ³lo por un nodo, por dos nodos, por tres nodos, etc.

 % Como se esperaba, gran parte de los dispositivos BT pasan pocas veces por casi todos los nodos, mientras que la mayorÃ­a pasa sÃ³lo por uno o dos de ellos (sus desplazamientos se centran en una parte de la zona pequeÃ±a monitorizada).
...
\subsection{Effect of Nov-14 strike on zone traffic}
 % Tabla en la que mostremos cÃ³mo afectÃ³ el dÃ­a 14-N con la huelga.

 % Justo en mitad del periodo de monitorizaciÃ³n, en EspaÃ±a se celebrÃ³ un dÃ­a de huelga general (14 de noviembre de 2012), lo que ha quedado reflejado en la cantidad de coches detectados en los nodos.

 % Se ha analizado el efecto de la huelga general en el trÃ¡fico de la zona monitorizada, mostrando en la Tabla \ref{huelga} el nÃºmero de pasos detectados el dÃ­a 14 de noviembre y justo el dÃ­a siguiente.
...
The effect of the general strike in the traffic of the monitored area has been analyzed and shown in Table \ref{huelga} as the number of detected devices on November 14 and the very next day.
 ...
 % ComparaciÃ³n del nÃºmero de pasos por cada nodo entre el dÃ­a de la huelga general (14 de noviembre) y el dÃ­a siguiente. No aparece recogido el nodo 1 debido a que el dispositivo hardware sufriÃ³ un problema de alimentaciÃ³n durante un par de dÃ­as en esas fechas
 \label{huelga}}
 \end{table}

 % En la Tabla \ref{huelga} se aprecia un nÃºmero de vehÃ­culos detectados menor el dÃ­a de la huelga respecto al dÃ­a siguiente (laborable en el que la actividad debiÃ³ ser normal en la zona).
...
\subsection{Analysis of vehicles speed between two consecutive nodes}
 % tabla de la velocidad media de los vehÃ­culos en el tramo (en laborables y en fin de semana).
 % El "Granada 5" estÃ¡ en el km 123,250 ; el "Granada 4" estÃ¡ en el km 119,550 ; hay 3700 metros entre ambos. 

 % Por Ãºltimo, a partir de los dos nodos consecutivos localizados en la autovÃ­a A44, podemos calcular velocidades medias en el tramo delimitado por los nodos 4 (situado en el km 119,550) y 5 (situado en el km 123,250) de un total de 3700 metros. 
 % En realidad podemos calcular la velocidad media en el tramo a nivel global, no la velocidad a la que ha ido cada vehÃ­culo en cada instante dentro del tramo.
...
v$\geq$140.0 & 1326  \\
 ...
 % En dicho tramo, la velocidad estÃ¡ limitada a 100 km/h. Sin embargo, y aunque la gran mayorÃ­a respetan el lÃ­mite, vemos que una gran cantidad de coches superan dicha limitaciÃ³n.
