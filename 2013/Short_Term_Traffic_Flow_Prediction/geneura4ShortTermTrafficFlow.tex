...
From the big variety of existing mtethods, 7 different ones have been selected in order to compare the validity of their predictions. The methods being used are the followings: ARIMA, Croston, Theta, Spline, the mean value, the random walk method, and, finally, L-Co-R. In order to compare results, a statistical study is included allowing us to draw some conclusions.

The experimentation has been carried out using the data collected for nodes 1 to 5 over 62 days. $75\%$ of the data collected has been used to train the methods, and obtain their associated models, and the remaining $25\%$ has been used to test their generalization capabilities. 
...
Methods  ARIMA, Croston, Theta, Spline have been executed using the R software, using the Forecast package (cita). In the other hand, L-co-R is a co-evolutionary algorithm based on radial basis function neural networks. The stochastic nature of the later lead us to run it 30 times per every node, so that average errors be computed.

The results yielded by every method have been compared using two non-parametric test: the Friedman test and the Iman-Davenport test. For both of them, only the Mean Absolute Percentage Error (MAPE) shows that there exist significant differences between the methods. Thus, the Friedman test yields a P-value of $0.04$ (according to a chi-square distribution with 6 degrees of freedom), and Iman-Davenport test yields $0.02$ (according to an F-distribution with 6 and 24 degrees of freedom).

The Friedman test also shows the significant methods among L-Co-R
