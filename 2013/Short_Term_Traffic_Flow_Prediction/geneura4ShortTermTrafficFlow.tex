...
Futhermore, Friedman test results also provide a mechanism to establish a ranking between the methods, determining which one can be considered the best. Once the method has been found out, it can be compared against the rest methods using the Bonferroni-Dunn procedure. This method computes the probability that two methods can be considered to have a similar behaviour, being this the null hypothesis. Thus, when the null hypothesis is rejected, methods can be considered to be significantly differents. Table ref{tb:bonferroni} shows the method that has been established as the best one for every error measure, and, with respect to this one, the methods for which the null hypothesis can not be rejected, and those that show meaningful differences.

Error MAE
Mejor L-Co-R
Iguales ARIMA, MEAN
Diferentes Theta, Croston, Spline
...
MASE
L-Co-R
ARIMA, MEAN, Theta, Croston
Spline

MdAPE
L-Co-R
Arima, Mean,
Theta, Spline, Croston

MSE
ARIMA,
L-Co-R, MEAN, Croston, Theta
Spline
