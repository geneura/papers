...
The lag selection is performed as a postprocessing stage in \cite{Maus2011} with a sensitivity computation of the output to each time lag. %The initial stage trains a single-layer, feed-forward ANN based on $d$ time lags, with $d$ chosen large enough to capture the relevant dynamics of the time series. %Another methodology which considers the lag selection as a post-processing stage is
