...
. This makes these architectures well suited to run large computational problems, such as those from bioinformatic area. 
...
\begin{figure*}[!h]
\centering
\epsfig{file=cpu-gpu.eps,width=\textwidth}
...
Hybrid models are those which does not match with the previous sections, because they mix some of the previous approaches using more than one or even introducing some novelty into the way of parallelization.  For example, the algorithms runs a coarse-grained deme model at the top level while each island or deme runs a master-worker or even another deme model with high migration rates. These approaches are not the most common because they need for additional new parameters for the topology and relationships of the populations; Moreover they need for hierarchical parallel architecture to host hybrid algorithms and they need expert knowledge because the implementation of these algorithms is a complex task. One of these hierarchical parallel architecture is the GPU.
...
%\end{itemize}
For the GPUs, hybrid EAs are perfect candidates to exploit the hierarchical memory and flexible block sizing in GPUs by well structured populations. 
...
% alguien tiene que leerselo todo y escribir unas conclusiones que no se parezcan a estas
...
\begin{figure}[hb]
\centering
\epsfig{file=logos_SIPESCA_2.eps,width=0.5\columnwidth}
\end{figure} 
