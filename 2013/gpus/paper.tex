...
Then, the goal of this article is to review the use of GPUs to solve such as problems, by means of the most extended metaheuristics\footnote{high-level procedures which follow strategies for guiding the search, in order to solve an optimization problem}, the \textit{bioinspired algorithms}. These algorithms imitate or model the behaviour of natural systems in order to design heuristic methods to mainly address search, learning, and optimization problems.
...
The rest of this paper is structured as follows: Section \ref{sec:parall_and_GPUs} presents GPUs as highly parallel devices architectures.
...
Focusing in the latter, this work is also a survey of the usage of General-Purpose GPUs for running bioinspired algorithms, implemented to solve large-scale or complex optimization problems. 
...
Most of the studied algorithms use the GPU for parallel fitness evaluation, but some other cases they present a parallel distribution scheme for the considered metaheuristic, such as Ant Colony Optimization or Evolutionary Algorithms.

In majority of the cases, extremely high speedups are attained in the comparison with traditional CPU sequential (and parallel) versions. Several authors agree on the bottlenecks that appear due to synchronization in the CPU parts of the algorithms, so new directions trying to minimize the communication between GPU and CPU are emerging.
