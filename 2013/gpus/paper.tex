...
\usepackage{amsmath}
\usepackage{epsfig}         % \usepackage[dvips]{epsfig}
\usepackage{graphicx}
...
\affiliation{Departamento de Arquitectura y Tecnología de Computadores, ETSIIT/CITIC, University of Granada}
\email{gustavo@ugr.es}
\shortauthors{G. Romero et al.}

\received{00 March 2014}
\revised{00 July 2014}

%\category{I.3.1}{Hardware Architecture}{Graphics processors, Parallel processing}
\keywords{Survey, Review, GPUs, General Purpose GPUs, Parallel Computation, Bioinspired Algorithms}
...
% eliminada por innecesaria... la dejo comentada por si alguien quiere que vuelva
%\begin{figure}[h]
%\centering
%\epsfig{file=./openclfigure2small.eps,width=\columnwidth}
...
% Antonio - he puesto una mini justificaci�n de por qu� se han analizado s�lo algoritmos de EC... mirad si es correcto
...
\item Parallel Genetic Programming (F. FernÃ¡ndez, et al.\cite{springerlink:10.1023_A:1021873026259}).
...
Most of the evolutionary computation metaheuristics can be implemented with small effort by using any of the existing frameworks. The most complete and comprehensive review of these frameworks is from Parejo \cite{springerlink:10.1007/s00500-011-0754-8}. It is a comparative study of metaheuristic optimization frameworks. As criteria for comparison, a set of 271 features grouped in 30 characteristics and 6 areas has been selected. These features include the different metaheuristic techniques covered, mechanisms for solution encoding, constraint handling, neighborhood specification, hybridization, parallel and distributed computation, software engineering best practices, documentation and user interface. The difference is that this survey just includes frameworks with GPGPU support only for evolutionary computation metaheuristics. 
% Antonio - esta �ltima frase no la entend�a y la he cambiado, pero no s� si lo que he puesto es correcto. En cualquier caso, �esto no es negativo para nuestro trabajo? Es como decir, nuestro art�culo est� muy limitado (s�lo EC), mientras que este de este colega es m�s chulo.
% 2) tambi�n se habla de ACO, luego no s�lo hay EC, �no?
% 3) features  y characteristics son lo mismo en casi todos los �mbitos... �en qu� se diferencian aqu�?

%%%%HHHHHHH 
%Maribel, esto es para gustavo. Si se incluye parte de frameworks en la primera parte del artÃ­culo, el siguiente pÃ¡rrafo deberÃ­a ir ahÃ­, sino, aquÃ­ la verdad es que tampoco pega nada y habrÃ­a que quitarlo creo yo.
...
\cite{Pedemonte:2011:BOG:2001858.2002031} studies the impact of using different representations for binary problems in GPUs to evaluate GAs: boolean data type versus packing multiple bits in a non-boolean data type. The execution platform was a PC with Quad Core Intel Xeon E5530 processor at 2,4 Ghz and a Tesla C1060 with 240 CUDA cores. The CPU only calculates the initial population and a matrix of random number to be used in crossover and mutation by the GPU. The authors review the problem of the data types when GPGPU is used. Several data types of 8, 16, 32, and 64 bits) are compared in CPU and GPU versions. Results shows that
...
Cano et al. \cite{SpeedingTheEvaluationofGPCano:2012} described a massively parallel evaluation model using a Genetic Programming algorithm for evolving rules for dataset classification. They copied the dataset to the GPU global memory, and evaluated the rules (individuals) for each instance of the dataset in parallel (match kernel). At the end, each successful match was reduced getting each individual fitness (reduction kernel). The implementation, based on CUDA, speeds up the fitness calculation phase and greatly reduce the computation time. Results were compared using one, two and four CPU threads and the combination of one or two GPUs of different features (285 cores for GPUa -NVIDIA GeForce 285 GTX- or 480 cores for GPUb -NVIDIA GeForce 480 GTX-). They test three classification algorithms and the results were not very significant for small datasets, but they increased the speedup with large datasets until 820x using two GPUs with 480 cores each one.
...
% Antonio - No se iba a revisar s�lo EAs? :P
...
In order to run a coarse-grained EA using a GPU, several kernels are run simultaneously, where each kernel handles a sub-population. This is not possible. 
% Antonio - decir por qu�...
This limitation of GPU would mean the conventional mechanics of coarse-grained EAs would need to be changed if GPU would be used.
...
% Antonio - revisa lo de los speedups, unas veces pone 'xN�mero' otras 'NumeroX' y otras sin la 'x'. Yo no s� la correcta... :P
...
This hybrid model just utilizes two or more of the master-slave, coarse-grained and fine-grained approaches in an hierarchical method. At the higher level, an island model algorithm runs while at the lower level, the demes (or sub-populations) are running in parallel (fine-grained, master-slave or even another island model with high migration rates). Hybrid models are not the most common in legacy EAs due to:
% Antonio - legacy??? no suena muy bien...
\begin{itemize}
...
In \cite{5586530} three cellular GA versions are compared: a CPU, a mono-GPU and a multi-GPU version, being the first work in use multi-GPU for cEAs. A 2D grid population is used and divided in two GPUs in the multi version. Selection, recombination, mutation and evaluation are performed in parallel. In the multi GPU case, a thread for each GPU is controlled by a CPU thread and borderline individuals of the sub-populations are exchanged. The speedup ranges from 8 to 711. However, there is not significantly difference between mono and multi GPU versions, probably due the overhead of the CPU. Hardware used is Intel Quad processor 2.67GHz and Nvidia GeForce GTX 285.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Maribel
%Falta incluir el otros dos artÃ­culos de Luong no sÃ© exactamente en quÃ© secciÃ³n cuyas referencias son \cite{luong:Neighborhood2010} y % \cite{LuongParallelHybridEvolutionaryAlgorithmsOnGPUCEC2010} FERGU: Maribel, ese ya lo he comentado yo con el \cite{5586403} lo quito de tu .bib
%Maribel: Ok, 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Maribel, este siguiente no me lo he bajado todavÃ­a, pero tambiÃ©n hay que ampliarlo.
% Antonio -  �este texto va con el anterior o est� suelto? 
Experiments running several instances of Fast Ant System \cite{Taillard:1998:FFA:870509} with the best neighbor at each iteration of the search and the QAP \cite{Zhu:2010:0020-7543:1035} are shown. 
% Antonio - el QAP ya se coment� previamente, aunqu eno s� si con cita
They use an Intel Core i7 930 with NVIDIA GTX 480 or the Intel Xeon E5520 with Tesla C1060. The GPU implementation is 15 times faster than the one running on the CPU.
...
Focusing in the latter, this work is also a survey of the usage of General-Purpose GPUs for running bioinspired algorithms (mainly Evolutionary Computation approaches), implemented to solve large-scale or complex optimization problems. 
Thus, a number of research works available in the literature have been reviewed, including a description of a taxonomy to classify them. 
...
In the majority of cases, high speedups are attained in the comparison with traditional CPU sequential (and parallel) versions. Several authors agree on the bottlenecks that appear due to required synchronization in the CPU parts of the algorithms, so new directions trying to minimize the communication between GPU and CPU are emerging.
