...
Many problems, such as large-instance or real-application ones, % ¿qué
                                % es un real-application problem? ¿Son
                                % exclusivas ambas categorías?
 require a high amount of computational power to be solved. In order
 ...
 quality of the solutions. Traditionally %cada vez que escucho
                                %traditionally Dios mata a un
                                %gatito. ¿Hasta cuando es
                                %traditionally? - JJ
these algorithms have been executed in central processing units
(CPUs). % hay ordenadores paralelos desde los 60 - JJ
However the advances in the videogames industry have led to the production of low-cost and high-performance graphics processing units (GPUs) included in almost every PC or even in every mobile device. Thus, some years ago, these units have been considered as general-purpose processors and profited as powerful substitutes or complements for CPUs regarding problem solving tasks.
The GPUs, in conjunction with CPUs, can offer a  higher speedup %over
                                %what? - JJ
than traditional CPUs, % por qué no dices que usarlas _aparte de la
                       % CPU_ puede resultar en una mejora de
                       % prestaciones? - JJ
so many researchers are migrating their algorithm to these systems. 
This work presents a review of hardware and software advances in this
line, along with their application in the area of some the most used
metaheuristics for solving complex problems: the bioinspired
algorithms.  % ¿Con qué objetivo se hace el review? ¿Para establecer
             % el estado del arte?  - JJ
...
handles computation only for computer graphics, % más bien di que está
                                % diseñada para ejecutar unas
                                % operaciones generalmente encontradas
                                % en gráficos - JJ
...
metaheuristics\footnote{high-level procedures which follow strategies
  for guiding the search, in order to solve an optimization problem} % esto no debería ser una nota - JJ
...
problems. % Si este es el tema del trabajo, deberíais justificar
          % porqué los elegís. - JJ
...
fall by a factor of two every year and a half.  % de hecho, yo diría
                                % que se ha parado ya - JJ
...
budget in mind in more common and older processes. %esta frase está en
                                %el artículo de 2012 - JJ
...
Traditionally CPUs and GPUs have been separated. % Enésimo error
                                % gramatical que corrijo. Por favor,
                                % miradlo bien - JJ
Now % ¿cuando? - JJ
