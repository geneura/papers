...
\title{Going a Step Beyond the Black and White Lists for URL Accesses in the Enterprise by means of Categorical Classifiers}
...
\abstract{Corporate systems can be secured using an enormous quantity of methods, and the implementation of Black or White lists is among them.
...
These CSPs usually include policies that allow or deny access to non-confident (or non-certified) web sites (referenced by their URLs in this work). Moreover, several web pages might be also controlled for productivity or suitability reasons.
...
Then, after analysing the yielded sets of classification rules and trees, it can be identified several rules based in other features rather than the URL itself, which is the aim of this work.
...
being 100000 patterns. We consider this dataset as quite complete because it contains a very diverse amount of connection patterns, going from personal (traditionally addressed at the first hour of work) to professional issues (the rest of the day).
...
% Antonio - Era la metodología,por eso estaba aquí en lugar de en el estado del arte. ;)
...
% Antonio - pos yo lo lo veo bien allí...
