...
\title{Applying Classification Methods to Manage\\ Secure URL Accesses in the Enterprise}
...
\abstract{Security management lists are normally used as a control
  tool in a distributed security system. % La pasiva debe usarse lo
                                ...
                                % los URLs?- JJ
 With these lists it is possible to restrict (or to allow) to the users the execution of applications or the access to URLs, among others. This paper is focused in the latter option. It describes the whole processing of a dataset composed by URL sessions performed by the employees of a company from the preprocessing stage, including labelling and data balancing processes, to the application of several classification algorithms. The aim is to define a method for automatically make a decision of allowing or denying future URL requests, considering a set of corporate security policies.
Thus, this work goes a step beyond the usual black or white lists, since they can only control those URLs that are specifically included in the lists (or the complementary ones), but not making decisions based in similarity (through classification techniques) as it is proposed here.
...
\noindent The new scenario defined by the Bring Your Own Device (BYOD) tendency, in which the devices that access to the system are owned by the users (company's employees), and could contain both personal and professional information, has turned the security focus to those users, who have become one of the main threats to the corporate security \cite{Opp_Security11}.

This has meant the rising of new security issues, which are normally
dealt by means of \textit{Corporate Security Policies} (CSPs), which
are %basically no uses basically en lenguaje formal -JJ 
a set of security rules aimed to protect the company's assets, by defining permissions to be considered for every different action to be performed inside the security system. 
...
One of the main issues in enterprise secury systems
% primer regarding - JJ
% Antonio - quitado
...
% websites or services - JJ
 is the access to non-confident (or non-certified) web sites (URLs in this work) ***REF***. Moreover, several webpages should be also controlled for productivity or suitability reasons.
Thus, some of the CSPs usually define sets of allowed or denied pages/sites that could be potentially accessed by the enterprise users/employees. These sets are included in the so-called White (permitted) or Black (non-permitted) Lists ***REF***. These lists offer a good control tool for those URLs included in them of for the complementary, i.e. the URLs non included in a White List have automatically denial access, for instance.
...
an allowance or denial decision with respect to URLs that are not included in the aforementioned lists. This decision would be based in the corresponding one to similar URL accesses (those with similar features).

Thus, the problem has been transformed into a \textit{classification} one, in which we have started from a set of unlabelled patterns, that model the connection properties from a huge amount of \textit{real} URL accesses (the so-called sessions). Then we have assigned a label to many of them, considering a set of \textit{real} security rules (CSPs) defined by the Chief Security Officer (CSO) in the company.
This dataset has been processed by means of different classification methods, in order to find the best algorithm for dealing with these data.
Previously data balancing techniques have been applied, namely \textit{undersampling} and \textit{oversampling} \cite{imbalance_techniques_02}, due to the high imbalance present in the dataset (more than double of patterns belong to the majority class).
...
\noindent Our work tries to obtain a URL classification for enhancing the security in the client side, as at the end we want to get if a certain URL is secure or not, having as reference a set of rules that allow or deny a set of known \textit{http} requests. For this, Data Mining (DM) and Machine Learning (ML) techniques have been applied. This section gives an overview in a number of solutions given to protect the user against unsecure situations.

The heuristic study of URLs for security purposes in the user side is not a novel practice. Also, the use of \textit{Blacklists} (in this work, the \textit{denied} URLs) and \textit{Whitelists} (\textit{allowed} URLs) are very extended practices. For instance, phishing is a problem of security that Sheng et al. \cite{} and Khonki et al. \cite{Khonji2001} tried to solve. The first work uses Blacklists as reference to avoid phishing attacks made by e-mail; the second one aims for an heuristic analysis of the URLs domain names and its ranks, in a way that a phished URL can be detected.

% Khonki et al. \cite{Khonji2001} phishing by heuristically studying the URLs.
% In "Highly Efficient Algorithms for Structural Clustering of Large Websites" they explain that distance can not be a good measure of URLs when there is a problem of classification.
% Blacklist -> An Empirical Analysis of Phishing Blacklists 
...
On the other hand, and more closely to our work, there exist some % Si
...
user-centric solutions to problems like user authentication in a
personal device, who Greenstadt and Beal \cite{cognitive_security_08}
proposed to address using collected user biometrics along with machine
learning techniques. % more closely, but not too close... - JJ
...
Then, when a security policy is going to be
applied, P.G. Kelley et al. \cite{user-controllable_learning_08} found
important to include the user in the machine learning process for
refining the policy model. They called it \textit{user-controllable
  policy learning}. Another approach to the refinement of user's
privacy policies has been described by Danezis in
\cite{inferring_policies_socialnetworks_09}, for he uses ML techniques
over the user's settings in a social network. % para hacer qué? - JJ

In the same line, Lim et al. propose a system
\cite{sec_policy_evolution_gp_08,pol_evol_gp_3_approaches_08} that evolves a  set of computer security policies by means of Genetic Programming (GP), taking again into account the user's feedback. Furthermore, Suarez-Tangil et al. \cite{rule_generation_gp_09} take the same approach as Lim et al., but also bringing event correlation in. These two latter author's works are interesting for ours, though they are not focused in company Information Security Policies (ISPs)  - for instance, our case with the allowed or denied http requests -.
...
The analysed data come from a typical \texttt{access.log} of the Squid
proxy application
...
origin server again \cite{DuaneWessels2004}.  % ¿Quién dice que es
                                % typical? ¿Es suficiente para probar
                                % lo que quieres? - jj
...
is numeric or nominal/categorical. % ¿Todas las variables son
                                % relevantes para la clasificación?
                                % ¿Por qué? ¿Quién lo dice? - JJ
...
being 100000 patterns. % y esto es suficiente porque... - JJ

Due to the nature of the data (URL accesses performed by humans), the
dataset is highly unbalanced \cite{imbalanced_data_05}. Thus, after a
labelling process described in Section \ref{sec:methodology}, there
...
belongs to the majority class. This is a very important problem of the
dataset, since a classifier trained considering these proportions will
aim to classify all the samples as ALLOW, getting a theoretically
quite good classification accuracy equal or greater than 68\%. % Eso
                                % no es bueno. Es una mierda - JJ

In order to deal with this problem there exist several methods in the
literature, % ... que deberías haber mencionado en el estado del arte
            % - JJ
but all of them are mainly grouped in three techniques
\cite{imbalance_techniques_02}: % Este no es el sitio de mencionar
                                % este tipo de cosas. Es el estado del
                                % arte. Si no está en el estado del
                                % arte, es que no tiene suficientes
                                % cosas - JJ
\begin{itemize}
...
On the other hand, the second option has been so far the most widely used, following different approaches, such as SMOTE (Synthetic Minority Oversampling Technique) \cite{smote_02}, a method proposed by Chawla et al. for creating `artificial' samples for the minority class, in order to balance the amount of them with respect. However this technique is based in numerical computations, which consider different distance measures, in order to generate useful patterns  (i.e. realistic or similar to the existing ones).
...
% Si esto es la metodología, ¿qué es lo que has descrito hasta ahora?- JJ
...
Furthermore, we have found that for the data sets taken consequently,
the methods always classify worse the DENY labels, as they label them
as ALLOW patterns. This worth futher study because it is the worst
situation. It would be preferable a false positive in a DENY pattern,
rather than a false negative and to permit a request that is forbidden
in the ISP. % podríais citar los trabajos de bancarrotas que hemos
            % hecho, donde se optimizan por separado los dos tipos de
            % errores - JJ 

% ¿No ponéis nada sobre los resultados? ¿Se limita a usar el dominio?
% ¿Hace alguna cosa más? ¿No podéis examinar el Forest ese, haga lo
% que haga¿ Si usa los dominios sólo, no es mucho mejor que el
% whitelist... - JJ
