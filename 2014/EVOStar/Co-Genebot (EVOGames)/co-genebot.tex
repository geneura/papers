...
Several experiments have been conducted in order to study different issues of the proposed approaches, but having in mind that the main objective is the improvement of bots using a co-evolutionary algorithm. The set of parameters considered in the Co-GA are show in table in Fig.\ref{tabla:GA}. 
...
\hline
Population Size & Generations & Crossover prob. & Mutation prob. & Elitism & Nts \\
\hline
...
\end{center}
\caption{Average time of execution}
\label{figura:tiempos}
\end{figure}

The second analysis concerns the fitness evolution. To this end the best, worst, and average (with standard deviation) fitness values are studied.
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[H]{0.49\textwidth}
        ...
        \end{center}
        \caption{Methods with previous knowledge: from top to down Turns/Position, Slope \& Area}
    ...
    \begin{subfigure}[H]{0.49\textwidth}
        ...
        \end{center}
        \caption{Methods with auto-generated knowledge: from top to down Turns/Position, Slope \& Area}
        \label{estudio:fitness:auto}
    \end{subfigure}
    \caption{Best, worst, average and standard deviation for fitness in every generation.}
...
\end{center}
\caption{500 simulations against 3 ANONYMOUSBots for each method.}
\label{figura:supertest}
\end{figure}
 
To study whether the bots actually achieved by the turn-based fitness are less competitive, we conducted a simulation pitting three bots achieved by each approach without crossing methods of knowledge. In Fig.\ref{figure:comparativa:fitness} can be seen that the fitness based in turns/positions is actually much worse than the other methods.
...
\end{center}
\caption{Results of 500 simulations matching the four best bots obtained (one per approach).}
