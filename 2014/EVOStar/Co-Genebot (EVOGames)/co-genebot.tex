...
Regarding the execution time of this approach, we did not expect a great time reduction during the training os the bots, although it was desirable. 
...
For this approach the knowledge the bot gets when it fights in previous battles is included into the individual, but the bots were improving for being better versus bots every time betters (because it's expected that the population will better in each generation). %Maribel, el will no tiene sentido, habla en pasado o en presente, no en futuro, es algo que ya está hecho no que se va ha hacer.
...
Regarding the execution time of this approach,  we expect to reduce the needed time by $50\%$ respecto to the previous approach, because it reduces the number of evaluation to the half.
...
In previous works, a bot is evaluated always versus the same bot (an reference-bot). For fitness function, the bot was evaluated several times (in different maps). The fitness function was defined depending of the result of the battle (if the bot wins all his battles or loses in someone) and the numbers of turns needed for end the game. For two bots A and B the fitness is defined like show in Fig.\ref{fig:fitness_turns_positions} \subref{fig:fitness_turns_positions:2}. 
