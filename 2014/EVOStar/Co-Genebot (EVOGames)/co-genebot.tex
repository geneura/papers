...
(EA), aimed to improve the behavioural parameters which guide the
actions of an autonomous agent (bot) inside a real-time strategy game
(RTS) named Planet Wars. % eso lo harías si se tratara simplemente de
                         % un algoritmo evolutivo, pero tienes que
                         % hablar de un algoritmo co-evolutivo - JJ
% Maribel- Deberías de decir qué poblaciones son las que estás teniendo en cuenta, %porque habrá al menos dos poblaciones no? 
...
%Maribel- Para qué lo vas a comparar? Será por que quieres obtener algo no? pues di para qué lo vas a comararlo.
% ¿Por qué la coevolución mejora los parámetros? ¿Qué te hace pensar eso? - JJ
To this end, 4 on 4 matches have been considered, being two of them potential agents (individuals) to evaluate.
*** The results show that the co-evolutionary scheme performs better
with regard the fitness convergence, as it was expected. %Maribel- No creo que decir que era lo que esperabas sea buena idea,
														% parece que eras adivino :-) 
														% Mejor dí sólo lo que ocurre, y nada de baticinios 
Moreover, the best bot obtained is a better opponent than our previous `champion',
winning against him in XX of YY matches. ***
% ¡no uséis with regard! - JJ
% maribel, por regarding sin el with
\end{abstract}
...
RTS games show an emergent component \cite{emergence_in_games2008} as a consequence of the cited two level AI, since the units behave in many different % maribel- Many different es redundante, pon sólo many 
...
The objective % Maribel-usa aim, objective está en deshuso 
of the game is to defeat all the starships in the opponent's planets. Although Planet Wars is a RTS game, this implementation has transformed it into a turn-based game, in which each player has a maximum number of turns to accomplish the objective. At the end of the match (after 200 actions, in Google's Challenge), the winner is the player owning more starships. 

There are two strong constraints (set by the competition rules) which determine the possible methods to apply to design % Maribel- Con design sólo vale no?
...
For more details, the reader is invited to revise the cited webs and
our previous work \cite{Genebot_CEC11}. % ¡¡¡¡Doble ciego!!!!! - JJ

%Maribel- a estas alturas, más o menos la mitad del paper todavía no has hablado nada de nada de coevolución ni de las poblaciones, ni de nada de nada de realmente el trabajo que has hecho
...
The competition restrictions strongly limit the design and
implementation possibilities for a bot, since many algorithms are
based on a memory of solutions or on the assignment of payoffs to
previous actions in order to % Maribel- el in order to no es necesario, ya casi no se usa, menos nosotros los españoles :-) pon sólo to.  
improve future behaviour. Moreover most of them are quite expensive in
terms of 
running time. Due to these reasons,  a set of rules which models the
on-line (during the game) bot's AI was defined. The rules have been
formulated through exhaustive experimentation, and are strongly
dependent on some parameters, which ultimately determine the behaviour
of the bot.  
...
Three type of bots are going to be tested in this paper, all of them
previously introduced in our previous work
\cite{Genebot_CEC11}\footnote{The source code of all these bots can be
  found at: forja.rediris.es/svn/geneura/Google-Ai2010}. % ¡¡¡¡¡Doble
                                % ciego!!!! - JJ

The first one is \textit{\textbf{GoogleBot}}, the basic bot provided by Google for testing our own. It is quite simple, since it has been designed for working well independently of the map configuration, so it may be able to defeat bots that are optimised for a particular kind of map. It just choose %Maribel- falta la s de la 3ª persona
a planet as a base (the one with most of its starships) and a target chosen by calculating the ratio between the growth rate and the number of ships for all enemy and neutral planets. It wastes the rest of time until the attack has finished. %maribel- haciendo qué? pierde el tiempo cómo. Creo que no pierde el tiempo sino que espera no?

% llámalo foobot - JJ
The second bot is known as \textit{\textbf{AresBot}}, and it was defined as the first approach for solving the problem. It models a new hand-coded strategy better than the one scripted in the GoogleBot. %Maribel- Pon los nombres en cursiva siempre. 
So, several rules were created based on the experience of a human player.
...
reinforce, if it already belongs to it in the next turns (since it can take some turns to get to that planet). %maribel- le sobra un to? 
...
a strategy quite fast would be able to beat it quite easily. That is %Maribel the reason?
...
% Maribel- usas ese reemplazo del mejor en la siguiente población incluso si en la siguiente población ya hay uno mejor?

The evaluation of one individual is performed by setting the correspondent values in the chromosome as the parameters for GeneBot's behaviour, and placing the bot inside \textit{five different maps} to fight against a GoogleBot. %Maribel- Cursiva. 
These maps were chosen for its significance and represent a wide range of situations: bases in the middle and planets close to them, few and spread planets, planets in the corners, bases in the corners, both planets and bases in the corners. % Haz mejor una lista y pones la razones de haber elegido cada uno de los mapas
The aim is to explore several possibilities %Maribel- Esto ya debería de estar super claro desde el principio y creo que es la primera vez que lo pones así de claro. Pero vuelves a no mencionar nada de co-evolución, todavía no sé como se usa. 
...
remain from the previous one, i.e. the elite. %Maribel- deberías decir porqué re-evaluas también la elite, y es porque su fitness puede cambiar dependiendo del adversario. Ya sé que lo dices arriba, pero aquí quedaría mejor y aclararía lo de evaluar a todos en todas las generaciones.
...
%is returned. 
% Si ya estamos a 12 páginas, me parece que os vais a tener que
% ventilar la mitad de las páginas estas - JJ 
...
In order to test the GA proposed in previous section, several experiments and studies have been conducted. These are different from those performed in the previous work \cite{Genebot_CEC11}%Maribel- Esto sobra, deben ser diferentes porque es otro paper
, being more complete in the first step (parameter optimisation), and analysing the pseudo-stochastic fitness function, and the value of the dealing mechanisms for avoiding it.
...
Due to the high computational cost of the evaluation of one individual (around 40 seconds each battle), a single run of the GA takes around two days %Maribel, ponlo en horas, días es una unidad muy grande
