...
In previous works, a bot is evaluated always versus the same bot (an reference-bot). For fitness function, the bot is evaluated several times (in different maps). The fitness function is defined depending of the result of the battle (if the bot wins all its battles or loses in someone) and the numbers of turns needed for ending the game. For two bots, A and B the fitness is defined as the Fig.\ref{fig:fitness_turns_positions} shows. 
...
In other cases, the sum of turns is called {$sturdy$}, and opposite to the {$ferocity$}, it is desirable a bot that take more turns in be defeated. In Fig.\ref{fig:fitness_turns_positions:2} there are an formal description of this fitness.
...
\end{center}
\caption{Number of ships of each bot in each turn} %Maribel, cambio if por of
...
For this case, the integral of the curve of the bot's live-line is used for calculate the area that is included into the fitness. This {$area$} is normalized to the number of turns, representing the average percentage of ships along the battle for each player.
...
Different experiments are development to studies each part of this paper. The principal experiment is improve bots using the co-evolutionary algorithm. The parameters uses in the Co-GA are show in table in Fig.\ref{tabla:GA} . For each previously presented method, at least 3\footnote{GA generally requires more executions, but executions are costly in time in our problem.} executions of the Co-GA.
...
\hline
\end{tabular}
\caption{Table with the parameters used in the GA.}
\label{tabla:GA}
\end{figure}

In addition, the time used for the evaluation in each generation was measure to compare the time needed for the GA. As you can see in Fig.\ref{figura:tiempos} the coevolution is faster than a simple GA, decreasing the time near to the half in the case of the previously-knowledge fitness, and a quarter in the autogenerated-knowledge fitness. 

\begin{figure}[h!t]
\tiny
\begin{center}
  \epsfig{file=./imagenes/tiempos_segundos.eps,width=7cm}
\end{center}
\caption{Generation average time of execution.}
\label{figura:tiempos}
\end{figure}

In Co-Evolution the fitness depends of the population, that means that a convergence can not be observe. Instead, the best, worts and average fitness in each generation is studied. In Fig.\ref{estudio:fitness} are represented these values and the standard deviation variation. The "best bot finders" execution for each fitness method and knowledge method are selected.

In autogenerated knowledge fitness (Fig.\ref{estudio:fitness:auto}) can be observer a homogeneous distribution throughout the generations, because the individuals are learning all of all so the difference between the population are equal all time.

However in the case of previously knowledge (Fig.\ref{estudio:fitness:previous}) in first generations a slight improvement in average fitness are observed. This may be because (on average) more and more individuals able to beat the bot used for prior knowledge exist.
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[H]{0.45\textwidth}
        ...
        \end{center}
        \caption{Methods with previous knowledge: Turns/Position, slope \& area}
    ...
    \begin{subfigure}[H]{0.45\textwidth}
        ...
        \end{center}
        \caption{Methods with autogenerate knowledge: Turns/Position, slope \& area}
        \label{estudio:fitness:auto}
    \end{subfigure}
    \caption{Best, worse, average and standard deviation for fitness in each generation.}
    \label{estudio:fitness}
\end{figure}

As fitness is different for each method, can not be compared (as they have different magnitudes). It is necessary to perform simulations in a wide range of maps and enemies to study how good they are.

First a fool bot is chosen to simulate 400 battles. In these battles, the bot obtained by GA have to fight against 3 of these bots. In the case of a fitness that does not favor (the spread) of the best solutions, our solutions come close to randomly generated solutions. For random solutions, fighting against these bots would not guarantee full-victories. In this paper, achieved a 100\% win for each method studied.

The next step is to study which of the methods creates a more competitive bot. For this it is necessary to submit to an ordeal all bots, and see what that comes off better. It is necessary that the test was very hard, to not get a full victories as in the previous study. 500 battles against three very tough opponents are simulated, showing in Fig.\ref{figura:supertest} in the results obtained. The best average results are obtained using previously knowledge. In both cases, the worst result is achieved using turns/position based fitness.
...
\tiny
\begin{center}
  \includegraphics[width=0.7\textwidth]{"imagenes/super_tabla"}
  \epsfig{file=imagenes/contra_3_genebots_grafica.eps,width=7cm}
\end{center}
\caption{500 simulations against 3 very tough opponents for each method.}
\label{figura:supertest}
\end{figure}
 
To study whether the bots actually achieved by the turn-based fitness are less competitive, we conducted a simulation pitting three bots fitness achieved by each without crossing methods of knowledge. In Fig.\ref{figure:comparativa:fitness} can be show that the fitness based in turns/positions is actually much worse than the other methods.
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[H]{0.7\textwidth}
        ...
        \end{center}
        \caption{With knowledge separately }
    ...
    \end{subfigure}
    \caption{Battle between best bot of each fitness methods in same knowledge type.}
    \label{figure:comparativa:fitness}
\end{figure}

Finally, the top four bots found are tested. This simulation is done by performing clashes between them. Fig.\ref{figura:todos_contra_todos} shows how the bots are evenly matched in strength. However, there is a subtle predominance of bots trained with slope based fitness. Similarly, bots trained with prior knowledge, perform better than those with self-generated knowledge.

\begin{figure}[h!]
\tiny
\begin{center}
  \includegraphics[width=0.85\textwidth, trim = 1mm 0mm 0mm 1mm, clip]{"imagenes/todos_contra_todos"}
\end{center}
\caption{Results of 500 simulations faced the best bots obtained.}
...
The first notable result is the significant reduction of training time needed using Co-evolution, reducing the number of simulations required.

It has been shown that a fitness-based game results, it is much less effective for training in co-evolution than one based on the study of curve resources (ships in this paper). Within these two fitness, based on the slope proved to be a little better, possibly to better represent the bots winners versus losers.

Also have been interesting to note that the use of previous knowledge can make a difference, although not significant. In the future it may be interesting to study whether the use of prior knowledge intercepts only the best solutions faster.

This paper opens up new avenues of research on the problem proposed. Among them include the study of different ways of extracting knowledge from the population for co-evolution, for example using for the fitness the Hall of Fame \cite{Nogueira_HoF2013} %(evaluar contra los X mejores individuos)
or the study of fitness suggested in other similar environments.

Finally, it would be interesting to study If at an CO-GA algorithm as the one discussed in this paper needs a fitness function. Opting instead to use the co-evolution to solve the crossover and propagation.
