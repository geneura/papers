...
The competition consisted in implement the best bot regarding the number of matches it won against the rivals.

Our first approach in this field was the so-called \textit{XXAnonymousXX-Bot} 
% Antonio - Poner XXAnonymousXX-Bot en lugar de GeneBoten todo el art�culo
\cite{Genebot_CEC11}, 
...
XXAnonymousXX-Bot was designed and optimized for 1 vs 1 battles, using a turn-based fitness function.  It evaluated every individual in the population by playing five different matches (in five different maps) against a sparring bot provided by the competition (GoogleBot), trying to avoid with these repetitions the noisy factor present in this problems (videogames) \cite{Mora_noisy_jcst},
...
In this paper, the aim is to face 4 vs 4 matches, trying to define an evolutionary approach which can improve the cited behavioural engine (set of rules) to this type of play. Thus, a co-evolutionary method has been proposed.
