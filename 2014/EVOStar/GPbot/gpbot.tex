...
\author{S. Ecret and A. Nonymous}%P. Garc\'ia-S\'anchez, A. Fern\'andez-Ares, A. M. Mora, P. A. Castillo and J.J. Merelo}
...
%esto no corresponde al
 ...
 Although this game has been described in previous works, we summarize saying that the objective of the player is to conquer enemy and neutral planets in a space-like simulator. Each player has planets (resources) that produce ships (units) depending of a growth-rate. The player must send these ships to other planets (literally, crashing towards the planet) to conquer them. A player win if is the owner of all the planets. As requirements, only one second is the limit to calculate next actions (this time windows is called {\em turn}\footnote{Although in this work we are using this term, note that the game is always performed in real time.}), and no memory about the previous turns must be used.  Figure \ref{fig:naves} show a screen capture of the game. 
...
\begin{itemize}
\item Can a tree-generated behaviour of an agent defeat a hand-coded by a player with experience and whose parameters have been also optimized?
...
RTS games have been used AMPLIAMENTE in the computational intelligence area (see \cite{Lara2013review} for a survey). %surey o survey ? FERGU: no lo había metido aun en el bibtex, cambio a review
...
\cite{Mora2012Genebot} the authors programmed the behaviour of a bot with a decision tree of 3 levels. Then, the values of these rules were optimized using a genetic algorithm to tune the strategy rates and percentages.  % ein????? - JJ - FERGU: re-escrito.
...
In this paper we use Genetic Programming to create the decision tree,
...
Age           & Best          &  \textbf{8.133} $\pm$ 3.95       & 5.467 $\pm$ 2.95       &    5,066   $\pm$ 2.11  \\ \hline  
   ...
   \multicolumn{5}{|c|}{Versus Exp-Genebot} \\ \hline             
 \multicolumn{2}{|c|}{}        &  Depth 3                & Depth     7            &   Unlimited Depth     \\ \hline  
...
As can be seen, versus Genebot, the average population fitness is nearest to the optimum than versus Exp-Genebot, even with the lowest depth. Highest PERMANENCIA in the population is also with the depth of 3 levels. On the contrary, confronting with Exp-Genebot the configuration with unlimited depth achieves better results. This make sense because more decissions should be taken because the enemy can be different in each map.

In the second experiment we have confronted the 30 bots obtained in each configuration again with Genebot and Exp-Genebot but in the 100 maps provided by Google. This has be used to validate if the obtained individuals of our algorithm QUE PONGO AQUI . Results are shown in Table \ref{tab:allmaps} and boxplots in Figure \ref{fig:victories}. It can be seen that in average, the bots produced by our algorithm perform equal or better than the best obtained by the previous authors. Note that, even obtaining individuals with maximum fitness (5 victories) that have been kept in the population several generations (as presented before in Table \ref{tab:results}) cannot be representative of a extremely good bot in a wider set of maps that have not been used for training. Because the distributions are not normal we have executed a Kruskal-Wallis test, obtaining significative differences in LO QUE DIGA PEDRO. Therefore, there are difference using a maximum depth versus Exp-Genebot, and therefore, the depth of the tree has influence.

To explain why results versus Genebot (a weakest bot than Exp-Genebot) are slighty worse than versus Exp-Genebot we have to see how the LO QUE SEA. Figure \ref{fig:gens} shows that best individual using Genebot reachs the optimal before Exp-Genebot, and also the average population converges quickier. This could lead to over-specialization: that is, the generated bots are over-trained to win in the five maps, and because re-evaluation these individuals are still changing after they have reached the optimal.
...
Configuration     &    Average maps won  &    Average turns     \\ \hline
 ...
 Depth 7          &   48.9 $\pm$ 10.21    &   141.386 $\pm$   133.916  \\ \hline
 Unlimited Depth  &   50.23 $\pm$ 11.40   &    15.54  $\pm$   10.55    \\ \hline
       \multicolumn{3}{|c|}{Versus Exp-Genebot}                          \\ \hline              
 Depth 3          &   52.367 $\pm$ 13.39 &  191.051 $\pm$ 67.79 \\ \hline
 Depth 7          &   58.867 $\pm$ 7.35  &  174.694$\pm$ 47.50 \\ \hline
...
This work presents a Genetic Programming algorithm that generates agents for playing Planet Wars game. A number of possible actions to perform and decision variables have been presented. We have used two competitive bots available in the literature (Genebot and Exp-Genebot) to use to set the fitness of the generated individuals. These bots were the best obtained from several runs and the behaviour to optimize was extracted from human expertise. Three different tree maximum depths have been used: 3, 7 and unlimited. Results show that best individuals outperforms these agents during the evolution in all configurations. We also have tested our agents with a larger set of maps not used for evaluation, obtaining results equivalent or betters that the previous agents.

In future work, other rules will be added to our algorithm (for example, the ones that analyse the map, as the Exp-Genebot does). Other games used in the area of computational intelligence in videogames, such as Unreal\texttrademark~ or Super Mario\texttrademark~ will be tested.
