...
For this work, two sample scenes or groups of archetypes have been
chosen: % why only two and why _these_ two? - JJ
The first one is called ``natality control'' scene and its goal is to model
a global archetype (population growth) and the appearance of four individual
archetypes (\textit{downtrodden}, \textit{helpless},
...
``revenge'' and its goal is to model the maximum appearance of an individual
complex memory-based behaviour between two characters (\textit{revenge}).


The Experiment 1, ``Natality control'' scene, aggregates different sample
archetypes where many factors must be taken into account.  It tries to find what number of profiles and values are optimal to:
\begin{itemize}
