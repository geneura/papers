...
% 4.- Se usarÃ¡ un GA para encontrar la mejor soluciÃ³n (conjunto de parÃ¡metros)
...
of archetypes: ``natality control'' and ``revenge''. A Genetic
algorithm (GA)
...
rat. We have modelled 4 states for it (be alive, be hungry, look for
mate and be pregnant), 7 actions (move, eat, attack, defend, escape,
find mate and have offspring) and parameters that define its
characteristics and probabilities to perform actions depending on the
state. It's important to remark that no ``feelings'' and no ``memory''
...
Figure~\ref{fig:lifecycle} represents on day in its life. Every
...
life-cycle of the Figure \ref{fig:lifecycle}. The execution of an
...
external) of their lives, and evaluate their interest.  % interest in
...
attack and defense behaviors). % Defínelos, hombre o di
...
\begin{itemize}
\item Ensure that, after 1000 virtual days, the  population that is
  still alive will be the 60\% of the total population. This is called a \textit{global archetype}.
\item Emergence of the \textit{downtrodden} archetype in a 22\% of
  the % Corregido este, corrige tú los otros. No sé si se puede
      % hablar de emergencia, puesto que lo has puesto tú a güevo - JJ
...
\item Emerge the \textit{warrior} archetype in a 22\% of the
  population. An agent will be considered as a \textit{warrior} if it
  has satisfactorily % qué significa eso? - JJ
 attacked at least five times. 
\item Emerge the \textit{helpless} archetype in a 22\% of the
  population. An agent will be considered as a \textit{helpless}  if
  it has been attacked at least ten times and hasn't defended the
  position. 
\item Emerge the \textit{bad warrior} archetype in a 22\% of the
  population. An agent will be considered as a \textit{bad warrior}
  if it has unsatisfactorily attacked at least ten times. % En casi
                                % todos sitios has usado satisfactory
                                % en vez de satisfactorily - JJ
\end{itemize}
...
This work presents the MADE environment, % no se presenta un entorno,
...
archetypes: ``natality control'' and ``revenge'' % which have been
                                % chosen because...
. We have used a
...
number of profiles is increased worse results are obtained. % This
                                % implies that... No queda claro la
                                % significancia del experimento. Si yo
                                % quiero usarlo para mi juego, ¿qué
                                % hago? ¿Y si quiero escribir una
                                % novela? 

In future works, more complex agents will be used, with different
rules to be modelled. For example, we plan to model more human
...
whose AI engine is publicly available for players and researchers. 
