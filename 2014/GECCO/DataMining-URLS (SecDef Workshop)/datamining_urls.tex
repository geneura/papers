...
\texttt{http\_reply\_code} & *** description *** & Categorical & 20 values\\
\texttt{http\_method} & *** description *** & Categorical & 6 values\\
\texttt{duration\_milliseconds} & Session duration & Numerical & integer in [***MIN, MAX***]\\
\texttt{content\_type} & *** description *** & Categorical & ***11 values (main content), 85 values (whole content)***\\
\texttt{server\_or\_cache\_address} & IP address & Categorical & ***XX values***\\
\texttt{time} & connection hour (in the day) & Date & 00:00:00 to 23:59:59\\
\texttt{squid\_hierarchy} & *** description *** & Categorical & 3 values\\
\texttt{bytes} & *** transferred number of bytes in the session *** & Categorical & 3 values\\
\texttt{client\_address} & IP address & Categorical & ***XX values***\\
\texttt{URL} & parsed URL (***TLD***) & Categorical & ***XXX values***\\
%\noalign{\smallskip} \hline\noalign{\smallskip}
