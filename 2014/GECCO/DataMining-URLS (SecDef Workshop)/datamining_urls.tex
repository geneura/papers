...
The analysed data comes from a typical \textit{access.log} of Squid \cite{squid_site}. This open source tool works as a proxy, but with the advantage of storing a cache of recent transactions so future requests may be answered without asking the origin server again \cite{DuaneWessels2004}. Thus, the studied log file has 10 fields that are described in the Table \ref{tabdata}. Also, it is indicated if the variable is nominal or categorical. Some of them are represented by numbers but that does not mean that they are nominal, but categorical, for instance, the client and server addresses. 
...
% Ránking:
% Nº- Técnica %acierto unders. / %acierto overs.
% Basados en reglas:
% 
% 1- NNge 96'49% / 98'76%
% 2- PART 96'45% / 97'54%
