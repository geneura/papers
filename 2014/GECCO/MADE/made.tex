...
backstories where desired archetypes (universally accepted literary symbols) emerge and their life stories are a by-product of the simulation.
...
To demonstrate the adequacy of the methodology, we perform an implementation using a specific multi-agent system and evaluate the results. % and the results are... - JJ
...
Games can include hundreds or even more NPCs, their collective interactions  improving the gaming experience and leading to a richer and more inmersive world. But inmersion is not guaranteed, but can only be achieved if the characters backstory is coherent and also diverse. % he añadido esto porque no había "Facts" a los que te refieres en la frase siguiente - JJ
which motivated us to develop a methodology to generate backstories by modelling the language, agents and a literary setting based in the concept of {\em archetypes}, behaviors and patterns universally accepted and
present in the collective imaginary. This methodology is able to 
create massive backgrounds % ¿fondos de pantalla? usad un lenguaje preciso - JJ
