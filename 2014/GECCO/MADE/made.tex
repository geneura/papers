...
% Mencionamos simulación, pero no la explicamos antes
\end{abstract}
...
present in the collective imaginary
 \cite{ArchetypesGarry05}. They allow to empathize  % mal escrito, y Â FERGU: arreglado. SÃ­, existe.¿existe empathize? - JJ
...
interesting for the player/spectator. %FERGU: dice antonio que pongamos aquí un par de técnicas en las que se basa MADE
...
\begin{itemize}
 ...
 \item Could the obtained results of our methodology be used to create a literary work? %FERGU: pregunta aÃÂ±adida que hay que contestar
\end{itemize}

%FERGU: Antonio propone esta pregunta en lugar de la última: Could this methodology be exported to be used in a bigger task such as creating a literary piece or work?

%Estas preguntas son las que tienes que contestar en las
%conclusiones. Si no, el trabajo ha fallado - JJ2013 FERGU:TODO mirad que estén contestadas al final
...
\cite{nairat2011character}. Story generation can be divided in two %FERGU: Dice Antonio si es 'storytelling'
...
the NPCs and objects in an interesting experience (from the dramatic point of view),
 which should be different in each execution, and adapted to the user's interaction. % Â¿QuÃ© metodologÃ­a usa cada uno? Â¿En quÃ© es mejor o peor que lo
...
%FERGU: Rubén, mira la frase del trabajo de Pizzi, que no se entiende muy bien, porque parece que es interactiva no-interactiva pero el usuario es espectador
...
the survey by Arinbjarnar et al. in \cite{ReviewArinbjarnar09}. %FERGU: describir algunos sistemas de este survey?
...
%ANTONIO: esta última afirmación parece falsa porque debes limitar el número para obtener algo, ¿no?
%Aunque lo acabo de pensar, sin saber bien todavía de qué va MADE, y supongo que al crear arquetipos cualquier nuevo personaje se ajusta a uno de ellos y ya está, por eso hablas de número no limitado, ¿no?
%En cualquier caso eso es MUY IMPORTANTE (uno de los selling points) y no está puesto ni en el abstract ni en la intro.
%Además de que habría que explicarlo un poco más para darle más bombo aquí. ;)



Following the ideas of the work by Epstein and Axtell
\cite{epstein1996growing} an environment based in {\em Sugarscape} has %Antonio: decir lo que es Sugarscape o quitarlo
...
obtain a plot (solution) where two characters interact in a creative way. %Antonio: Entiendo que esto es nuevo, no lo que hizo ya el tal Nairat,¿no? Queda un poco lioso y puede parecer que esto ya lo hizo ese menda, cuando yo creo que tú quieres decir que os basásteis en su trabajo para hacer esto, que él no hizo. ;)
...
%TODO: presentar la metodología del párrafo siguiente
...
To validate our methodology we apply their steps using a specific scenario and environment. In this scenario we want to model the next story: %FERGU: IMPORTANTE!!! HE PUESTO STORY, LE PONEMOS OTRA COSA? HAY QUE CUADRAR TODOS LOS CONCEPTOS (VER LISTA ARRIBA)
...
as a fitness a function that model the literary setting. %Results show that in the first archetype the number of
