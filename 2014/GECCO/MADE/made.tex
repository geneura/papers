...
\title{A methodology to emerge literary plots using genetic algorithms} %FERGU: pensar un tÃ­tulo que no de vergÃ¼enica, este me lo acabo de inventar 
...
%FERGU: HABLAR DE LA METODOLOGIA COMO UNA NARRACION
These facts have motivated us to develop a methodology to...

First, the creator should define a set of agents in a virtual environment...

The literary setting is modelled using the concept of {\em archetype}. The archetypes are behaviours and patterns present in the collective imaginary
 \cite{ArchetypesGarry05}. Archetypes allows to empathize  % mal escrito, y ¿existe empathize? - JJ FERGU: arreglado. Sí, existe.
...
%FERGU: definir aquí los perfiles mejor
...
% FERGU: añadido arriba, Ruben, completalo que tu lo dominas más
...
\begin{itemize}
 ...
 \item Could be the obtained results of our methodology used to create a literary work? %FERGU: pregunta aÃ±adida que hay que contestar
\end{itemize}
...
% (JJ2013): Decir quÃÂ© te hace suponer que esto va a ser una buena soluciÃÂ³n.
% (RubÃÂ©n): TODO

Previous works define the plot as an emergence for the behaviour of the agents that follow a set of rules. In MADE, the agents' behaviour is product of their personality and the environment. That is, the agents does not follow the plot, but they generate the plot itself.
...
Once the agent have been defined they can behave in different context. For example, we can define a love story between them, or maybe a traition.

\subsection{Define GA}


It is important to clarify that the term ``individual'' in this work refers to a solution in the EA that models a whole environment, where a number of different ``agents'' are living. So, an ``individual'' in the EA is not equal to an ``agent''.  %FERGU: dejo esto claro para que no se lien los revisores
...
To validate our methodology we apply their steps using a specific scenario and environment. In this scenario we want to model the next story: %FERGU: IMPORTANTE!!! HE PUESTO STORY, LE PONEMOS OTRA COSA? HAY QUE CUADRAR TODOS LOS PARÁMETROS (VER LISTA ARRIBA)
 ``A number of rats live, eat, reproduce, compete for food and death within the walls of the Invisible University of Ank-Morpork\footnote{This is our tribute to writer Terry Pratchett, whose books have inspired us to create these agents.}. As the University professors, these rats are very vindictive.'' %
...
% (JJ2013): Sequentially de quÃ¯Â¿Â½ secuencia? - JJ2013
% (RubÃÂ©n): TODO
...
% (JJ2013): Usas un estilo demasiado conciso. Di quÃ¯Â¿Â½ hace cada uno de esos
% parÃ¯Â¿Â½metros, cÃ¯Â¿Â½mo se meten (fichero de configuraciÃ¯Â¿Â½n o como sea), por
% quÃ¯Â¿Â½ se eligen esos y no otros, y los valores por defecto que has
% usado.

% (RubÃÂ©n): por simplificar y reducir tamaÃÂ±o, pongo los valores por omisiÃÂ³n
...
%metodologÃ­a. DeberÃ­as justificar estos estados y las acciones. Â¿Se
...
agent to perform the actions described in the item bellow. % No se
                                % usan números Cambiar abajo - JJ FERGU: OK
...
characteristics (probabilities to perform different actions). % Los
                                % agentes son un FSM entonces, Â¿no?
                                % Cita los papers en los que hablamos
                                % de FSM para Mario o Unreal, por ejemplo.
                                % Â¿Esto es fijo? - JJ2013



% (RubÃÂ©n): TODO ÃÂ¿citar todas las caracterÃÂ­sticas de un agente made?
% SÃÂ³lo si hay hueco, creo
% No se ve nada y no aporta mucho. No uses notaciÃ¯Â¿Â½n del programa, sino
% del algoritmo. Y mejor que lo contaras como un algoritmo usando el
% entorno algorithmic que este grÃ¯Â¿Â½fico. - JJ2013

% (RubÃÂ©n): TODO No se cÃÂ³mo mejorarlo...

% CÃ¡mbialo por un algoritmo. Ocupa demasiado espacio y no se ve nada.
% O bien dibÃºjalo como un autÃ³mata de estados finitos en vez de un
% diagrama de flujo. Un redondÃ³n por estado y flechas de transiciÃ³n -
% AdemÃ¡s, me da la impresiÃ³n de que estÃ¡s mezclando estados de un
% bicho (pregnancy) con los de otro (a partir del nacimiento). Si es
% asÃ­, mÃ¡s que pregnancy deberÃ­as llamarlo "embryo" (y, sinceramente,
% no sÃ© si merece la pena mencionarlo aquÃ­ y quÃ© interÃ©s puede tener)
% JJ2013
...
For each agent, if the agent's log matches the archetype, it adds one point to the fitness. Therefore, the goal is to recreate an environment where most ``avenger'' agents exist.
...
% Â¿QuÃ© son los perfiles? No lo has definido en ningÃºn sitio. DeberÃ­as
% ponerlo tambiÃ©n en el abstract si es tan importante - JJ2013
% Sigue sin estar claro, ni qué relación hay entre perfiles,
% arquetipos, número d elos mismos ... igual un gráfico ayudaría. O un
% ejemplo - JJ FERGU: añadido arriba (aunque hay que definir bien los perfiles)
...
to the agents in order of appearance in a loop.  It's
...
% Â¿QuÃ© hay que ver aquÃ­? Comparar un nÃºmero de perfiles con otro? - JJ2013
...
% (JJ2013): Por quÃÂ© has elegido estos experimentos y no otros? - JJ2013
% (RubÃÂ©n): Done
...
%attack and defense behaviors).
                                % DefÃ­nelos, hombre o di
                                % algo de ellos, como "identified
                                % by". Â¿Se supone que el lector
                                % sabe lo que son? Â¿QuÃ© tiene que ver
...
% (RubÃÂ©n) TODO: ÃÂ¿Es interesante poner las expresiones regulares?
...
%\*** Average population fitness  at the end of each execution \\
% Igual, pero la media del fitness medio en la ÃÂºltima generaciÃÂ³n.
% Vamos, lo copias de la hoja de cÃÂ¡lculo, lo que estÃÂ¡ en negrita.
%\end{center}
%\end{table}
...
%Lo mÃ¡s importante: responde a las preguntas que te has planteado? - JJ2013
