...
In a world with such a number of characters, their collective interactions could improve the gaming experience, leading to a richer and more inmersive world. These facts have motivated us to develop a methodology to generate backstories by modelling the language, agents and a literary setting based in the concept of {\em archetypes}, behaviors and patterns universally accepted and
...
the first widely known multi-agent generative social model. As a step of the methodology, a self-organizing system if defined following the methodology introduced in
...
giving freedom to creators. This issue has been studied in the systems presented of 
 survey  \cite{ReviewArinbjarnar09}.
...
To validate our methodology we apply its steps using a specific scenario and environment: the rats that inhabit in the Invisible University of Ankh-Morpork\footnote{This is our tribute to writer Terry Pratchett, whose books have inspired us to create these agents.}.  %the next story: 
...
In this paper we model a virtual rat with four states (be alive, be hungry, look for
...
it tries to find the number of profiles and values which are optimal to make \textit{revenge} archetype emerge in as many agents as possible.  An\textit{avenger} if it has been attacked by other agent and after that, in a moment in its life, it has satisfactory attacked the agent b, in \textit{revenge}. For this setting, every agent whose log matches the archetype adds one point to the fitness. %Therefore, the goal is to recreate an environment where several ``avenger'' agents exist.
...
\subfigure[Setting ``Territorial war'']{
   \includegraphics[scale=0.3] {img/exp1_v2.pdf}
   \label{fig:subfig1}
 }
\subfigure[Setting ``Revenge'']{
...
as fitness a function that models the literary setting.  The execution of the MADE Environment
