...
\crdata{978-1-4503-2881-4/14/07\ ...\$15.00.\\
...
J.J. Merelo, Pedro Castillo, Antonio Mora\\
       ...
       \affaddr{GeNeura, Department ATC, ETSIIT + CITIC}\\
...
\alignauthor
V\'ictor Rivas-Santos\\
\affaddr{Universidad de Ja\'en}\\
...
\maketitle
\begin{abstract}After existing for more than fifteen years, JavaScript has finally risen as a popular
...
\keywords{JavaScript; {\tt node.js}; parallel evolutionary algorithms;
  asynchronous evolution; implementation issues}
...
language for the browser, and in fact most books on JS start by
...
open source license and are available at \url{https://github.com/JJ/nodeo}. They offer a glimpse into the different
...
\cite{langdon2009evolving} and claims:
\begin{quote}
...
manager just by issuing {\tt npm install nodeo}.
...
% the values shown in Table \ref{tab:trap}, 
which are the usual in other
...
% Sólo 2 millones, y tarda unos 20 segundos. Es rápido. 
...
\begin{figure}[htb]
\centering
   \includegraphics[scale=0.35]{base-hits-pop}
...
\begin{figure}[htb]
\centering
   \includegraphics[scale=0.35]{base-time-pop}
...
\begin{figure}[htb]
\centering
   \includegraphics[scale=0.30]{benchmark}
\caption{Benchmark comparing running time for {\tt node.js} and two
  browsers. Node version is 0.10.29, Firefox 26.0 and Chromium
  31.0.1650.63, all of them running on Ubuntu 13.10. The first
  benchmark has been repeated 30 times, and the others 10 times. All
  results run the same benchmark, with the node version using the same
data as that plotted in Figure \ref{fig:time}; the benchmark uses
4-trap with 30 blocks and population = 512.}
\label{fig:benchmark}
\end{figure}
%
...
frameworks. We have also tested different JS virtual machines to see
if there is a difference between implementations. The fact that it is
done in JavaScript allows to easily transform Node code into one that
can be run in the browser by just using a tool called {\tt
  browserify}; the resulting code is, in fact, the same, with the main
difference being the way it is called and the fact that it has to be
done from a web page. There are also slight differences in the way the
loop is done, or rather not done: it has got to stop from time to time
to avoid blocking the browser. Even so, benchmark results shown in
Figure \ref{fig:benchmark} show that performance of all virtual
machines is essentially the same, with a statistical significant
difference only between Firefox and Chromium, not between Node and the
other two implementations. 
% Antonio - quizá haría falta una comparativa con otras
% implementaciones o una REF para aseverar esto
%metida en la nueva versión. 
...
% Antonio - Interesaría hacer una breve intro a los modelos
% distribuidos de EAs, o un par de citas. No todo el mundo en el GECCO
% los conocerá, creo yo. ;)

%Al que no los conozca- lo echamos - JJ
...
\begin{figure}[htb]
\centering
   \includegraphics[scale=0.35]{parallel-hits-pop}
...
\begin{figure}[htb]
\centering
   \includegraphics[scale=0.35]{parallel-time-pop}
...
evolutionary algorithms. In an environment with no constraints to
choose a particular programming language, JavaScript
will probably be slower than Java or C++, although its speed is on par
(and may beat) other scripting languages. However, in an environment
...
mobile applications (which can easily be created in JavaScript via,
for instance, the
...
parallel evolutionary algorithms, as we have proved in this paper.  
...
testing uniform vs. heterogeneous settings in nodes. We have been using a
  ...
  diversity and thus increase the success rate; heterogeneity could
  come from parameter settings or from different machines being
  used. Besides, 
...
the P2P and pool-based options are not mutually exclusive, since
...
This paper has been developed thanks to the support of the projects  CANUBE (CEI2013-P-14), ANYSELF (TIN2011-28627-C04-02) and Muses (\url{http://musesproject.eu}).
