...
Previous generation algorithm is combined with the implicit fitness evaluation for selection and replacement. This evaluation of implicit fitness is performed confronting the individuals in a real-scenario. Selection of the mating parents is performed in a battle: the winner is selected to crossover and the loser will be removed from the population (as it will be explained later).
...
\end{center}
\caption{Score vs GPBot. Casi igual que las anteriores pero en boxplots SIN outliers (por eso la del best individual parece que el máximo está en 5, con lo cual no debería estar en 23 en la que son líneas). Estas dos figuras puedes quitarlas si quieres.}
...
\end{center}
\caption{Victories vs GPBot (all maps). Muestra el \% de individuos que ganan 0,1,2,3,4...30 veces a GPBot al principio, y debajo otra al final. Puede verse que al principio hay muchos individuos que no ganan ni una sola vez a GPbot. Al final se puede ver que la cosa está más repartida (recordemos que los individuos se han generado peleándose entre sí)}
