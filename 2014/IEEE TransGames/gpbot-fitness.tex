...
% "Struggling to survive" está mejor dicho - JJ FERGU: Struggling,
% pues, que lo he visto en pelis

% ¡que no me gusta el without, coñe! Que lo quitéis y pongáis una
...
% Antonio - Sí, mejor con una cita FERGU: añadido
...
% Antonio - Poner cita a los trabajos anteriores de GP en Planet Wars (nuestros o de otros) FERGU: están puestos más abajo
...
% Antonio - esto no sé si es verdad FERGU: sí es verdad, pero al final no sale tan bueno este bot contra ellos...
...
% Antonio - no, la cita que decía de los co-evolutivos. Ponla porfa XD
% Antonio - [TODO] De hecho habría que darle mucho más bombo a los co-evolutivos en el artículo. Hablar un poco de ellos desde la intro, ya que esto es 'a shape of Co-evolutionary approach'. No lo pongo en la para no descuajaringar nada, pero estaría bien. Máxime cuando en los experimentos sí que te refieres al método como Co-GA. OJO también con eso. ;)
...
% Antonio - pon cita. Mira el artículo de antares en el EVO* en el que metí un párrafo chulo de Co-evolución con citas y demás. Pon lo que veas en la intro o aquí, pero hay que poner más de este tema. ;)
...
% Antonio - El árbol lo ejecuta un bot para decidir qué hacer considerando un planeta, no lo ejecuta un planeta, ¿no?. Lo he intentado escribir así. FERGU: lo he reescrito yo también para que sea más entendible
% Antonio - mira a ver si esto es verdad, Fergu. ¿se decide el porcentaje también aquí, ¿no? FERGU: reescrito todo
\end{itemize}
...
% esta frase queda como "descolgada"... no entiendo a qué viene nombrar el Alg. 1 aquí (al menos de esta forma).   [pedro]
...
% Antonio - e.g. significa 'por ejemplo'. ¿No querrás decir i.e. que significa 'es decir'?
...
% Antonio - revisad que esto es así (se asegura que no se eligen los mismos individuos como padres).
...
% Antonio - Decir qué tipo de cruce se ha implementado
Then, the mutation operator is applied...
% Antonio - decir qué operador de mutación se ha usado y justificarlo
...
original, % será random, no stochastic. Stochastic se es o no se es,
          % es como estar embarazada - JJ
...
% Antonio - no se incrementa la diversidad por seleccionar padres aleatorios, ¿no? Se incrementaría si se eligiesen malos individuos como padres. ;D
...
% Antonio - falta darle un poco más de bombo a esto, parece una chuminá, pero en realidad... ¡ES LA POLLA! :D
...
These parameters have been  previously used in \cite{GarciaGP14}, obtaining competitive bots. To do a fair comparison with the results obtained in that work the termination criteria has been also set to 8000 battles (therefore, 4000 generations). 30 executions of the Co-GP have been performed in order to obtain accurate statistics.    % completo la frase  [pedro]
% Antonio - cuidao que unas veces pones Co-GP y otras Co-GA
...
\end{equation}
% Antonio - no pongas esto como ecuación. Es sólo una aclaración de un rango
...
%Será score SurvivalBot vs. gpbot, ¿no? - JJ FERGU: sí, es que era un
%copypaste de mi mail. Arreglado caption, tamaño y quitado título
% En mi versión sigue con el mismo título. El eje horizontal debería
...
% Antonio - cuidao, que no se ha dicho antes que sea un co-evolutivo y no se han usado esas siglas.
...
% son muchas - JJ
% Y seguís sin hacerme caso a este comentario. Será Histogram of
% number victories for the algorithm population o algo así - JJ
...
% Y enlazar con el siguiente, no me gusta nada el comienzo - JJ
...
(attack, reinforce and do nothing). As it can be seen, %no digáis
                                %nunca As it can be seen. Se prueba o
                                %no se prueba, pero nunca se
                                %ve. Además, ni siquiera habéis puesto
                                %una referencia - JJ
 the {\em do
  nothing} action has been increased % la acción se ha incrementado?
                                % Qué significa eso? Será que el
                                % númeor de veces que aparece esa
                                % acción nosé dónde lo ha hecho, ¿no?
                                % - JJ
...
\end{center}
\caption{Distribution of different types of actions (attack, reinforce
  and do nothing) of the generated bots.}
% Estos gráficos están fatal. Las etiquetas están cortadas. Y no están
% etiquetadas cada tarta qué significa. - JJ
...
\caption{Owners of target planets when attacking: Enemy, Neutral,
  NotMy.} % Será notmine, ¿no? No notmy. 
...
\end{center}
\caption{Destination of planets when reinforcing: Weakest, Wealthiest, Base.}
...
\hline
{\em Bot Name} & {\& Reference} & {\em Number of simulations} & {\em Max. Turns} \\\hline \hline
