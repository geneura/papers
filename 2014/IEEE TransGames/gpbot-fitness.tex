...
against another type of enemy \cite{wilcoxon:ga,Genebot_JCST}. 
...
The experiments and results are described and discussed in Section \ref{sec:experiments}, where the obtained bots are analyzed and compared with other bots available in the literature. Finally, the conclusions and future lines of research are presented in Section \ref{sec:conclusions}.
...
One of the shortcomings of previous works is that they depend on a baseline bot, taken as rival during the evolution, or an ad-hoc fitness function that involves some kind of parameterization (for example, number of matches or scoring the actions) \cite{Genebot_JCST}. To avoid this, co-Evolutionary Algorithms (CEAs) 
...
\item {\em Action}: a leave of the tree (therefore, a ``terminal''). Each decision is the name of a function, and a ratio between 0 and 1. The function indicates to which planet send a percentage of available ships in the planet (from 0 to 1). As the bot executes the tree one time per planet each execution uses the information of the current planet.
