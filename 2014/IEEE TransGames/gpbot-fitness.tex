...
that eschews an explicit fitness function, and substitutes it by a match between individuals, which we will call {\em joust}, % podï¿½a quedar hasta bien
                                % - JJ
implemented as a selection mechanism. Instead of measuring fitness by
making the bots perform certain tasks or fight against baseline
rivals, they fight in a co-evolutionary shape % shape es una forma
                                % geométrica. "In the shape of a
                                % rectangle". Queréis decir "way",
                                % pero tampoco está bien
                                % dicho. Además, usáis unas veces
                                % co-evolutionary y otras
                                % coevolutionary. 
with other individuals in the population. The winner will  survive, being moved  to the next generation. Thus, explicit evaluation is replaced by implicit comparisons between bots. This algorithm has been designed as an optimization approach to generate the behavioural engine of
...
Section \ref{sec:survival_bots} presents the proposed method, termed
{Survival Bot}. %Survival bot suena fatal. Superviviente es que
                %sobrevive de una catástrofe, no de una lucha, que se
                %llama simplemente vencedo. En todo caso "survivor",
                %no survival, y mucho mejor "gladiator" o algo
                %así. Pero tampoco está bien porque no es un bot
                %único, sino muchos generados por un método. - JJ
...
Evolutionary Computation (EC) has been applied in a wide variety of
issues inside the videogames scope. One of the most profiting % ¿qué
                                % es most profiting? ¿No será
                                % promising? - JJ
areas inside them is the parameter optimisation of behavioural engines \cite{Mora-Evo2010,cooperativebots_CIG2010,Genebot-IWANN2011}.
...
% Lo que no viene a cuento es que cites sólo tres trabajos tuyos. 
%
...
%which explains the title we have given to this paper.  % Esta frase
%casi que la quitarï¿½a  [pedro] FERGU: ala, fuera
% single match? Which map? Esta sección es FUNDAMENTAL y tiene pocos
% detalles - JJ
Thus, the selection of each one in the two mating parents is performed  in a battle. The winner of the matches is selected to mate
...
The use of such a selection/survival process tries to reduce the noise
added by the fitness evaluation in the evolutionary process
\cite{Genebot_JCST}. So, the individuals which are not able to win in
a match are strongly penalised, and thus, removed from the population
of the next iteration. 

% Ahora tendréis que explicar cuál es la diferencia con un torneo
% normal y corriente, en el que cada uno de los individuos obtiene un
% fitness al enfrentarse con un tercero. Además, la función de fitness
% no _añade_ ruido. El proceso de obtención del fitness en sí es lo
% ruidoso. Además,  una sola evaluación (y, además, en cinco
% mapas) es igual de ruidoso, o menos, que una sola lucha. ¿Y si
% luchan dos veces y gana uno cada vez? - JJ 
...
% Because no lo habéis contestado. Además, tenéis que especificar más
% cuantos sustituís y por qué. El proceso steady state tendería a
% reducir el ruido, porque los que se quedan en la población se
% enfrentan varias veces, aunque con que pierdan una la palman. Así
% que habría que ver qué efecto tiene el hecho de que se sustituyan
% un porcentaje u otro - JJ 
...
due to the lack of a fitness value which can value every individual
with a simple number. The random selection of all the individuals also
increases the chance of reducing the presence of noisy bots,
i.e. those which are not good enough to remain in the population. % si
                                % es bigger random, ¡es más ruidoso!
                                % ¡Y se supone que lo que estáis
                                % queriendo hacer es disminuir el
                                % ruido! - JJ 
...
% Antonio - lo del 4vs4 FERGU: comentado
    ...
    \STATE {\em /* Previously selected bots do not participate again
      in the tournament */}
% Esto no se dice con un comentario, se incluye en la población - JJ 
...
Thus, in order to do a fair comparison with that method, the termination criteria in SurvivalBot has been set to 8000 battles (therefore, 4000 generations/iterations), since GPBot considered 32 individuals * 5 combats per evaluation * 50 generations = 8000 evaluations/combats. 
...
The last study in this set of experiments concerns the age of the
individuals. This can help us understand how the evolution is has been 
% is has. Vaya tela. - JJ 
performed. Figure \ref{figura:age} shows the ages of the individuals
during one run. It is interesting how the age has a limit and does
not increase during all the run, meaning that a truly good bot is not
generated at the very beginning and remains during all the
evolution. % interesting por qué?
This tends us to think that, again, the whole population is
% ¿Nos tiende a pensar? Por favor... - JJ 
effectively improved so, the good bots are beaten by the offspring a
few generations after. The extreme values which get up to 50
% Vamos, que evoluciona. Si no evolucionara, estaríamos apañaos. 
generations happen due to the random selection of contenders, which
could omit a bot to fight for several generations. % Con esto vienes a
                                % decir que uno de los que esté en la
                                % última generación puede estar ahí
                                % por casualidad - JJ 
...
%Decir qué queréis sacar de aquí. 
All the SurvivalBots obtained at the end of the runs have been
initially tested against other bots available in the literature.  

To this end, jut one SurvivalBot per run must be chosen, so first the
{\em best} individual of every run has been selected by confronting
all the individuals of the last generation in an \textit{all vs. all
  tournament}. The bot who has won more times is considered as the
best. This method has been considered in order to avoid the usage of
the score function (and therefore, the shortcomings we are trying to
avoid). % ¿En qué quedamos? ¿All o one of them? ¿Cuántas veces los
        % enfrentáis 1 vs 1.? ¿En qué mapa o mapas? ¿Por qué evitas la
        % función de scoring? La última fase sobra. 

Then, we have confronted the 30 best bots obtained in each
configuration % ¿Configuration? ¿De qué estás hablando? ¿El mejor de
              % cada ejecución? A ver, no será más fácil decir algo
              % así:
% Since there is no explicit fitness, the population obtained in the
% last generation is not ranked; if we want to choose the best among
% them we will have, again, to perform a joust all versus all to
% obtain the best among them. 
again with several bots available in the literature, in the 100
example maps provided by Google with the competition framework. These
have been used to validate if the obtained SurvivalBots can be
competitive in terms of quality in maps not used during evolution, and
against unknown bots (as a difference to the other approaches). % ¿Qué
                                % other approaches? No has comparado
                                % con ninguno, ¿no? - JJ
Table \ref{tab:literaturebots} presents the bots used as opponents. 
...
% simulations in training? ¿Max Turns?- JJ
...
of the percentage of victories of the SurvivalBots against every
rival. Note that it only shows the victories, not the draws. The most
interesting result is that GPBot is clearly outperformed, even if the
number of battles has been the same to train SurvivalBot than for
GPBot, as we set. Therefore, our method can generate competitive bots
without using existing ones during the training phase. % y esto es
                                % bueno porque... - JJ
 The HoFBot, which was also obtained using co-evolution, has also been
 defeated more than 50\% times by most of the best
 SurvivalBots. However, highly trained bots (GeneBot and ExpGenebot),
 which applied 4 times more evaluations to be generated have been
 difficult to beat. It is interesting to mention that in
 \cite{GarciaGP14} GPBot obtained better score when confronting them, but this happened because they were used to train it, so it
 was focused only in beating them. % ¿Y si estos dos necesitan más
                                % ciclos, ¿por qué no usas más ciclos?
                                % - JJ 
...
against existing bots in the literature. Averages are taken over the
30 SurviBots, which are the beast in the 30 runs.}
\label{figure:boxplot_mejores_contra_clasicos}
\end{figure}

% A ver, este es el resultado principal del paper, y no es tan
% bueno. Con dos bots publicados, y además publicados por ti, obtiene
% peores resultados. Además, te escapas diciendo que es porque usa más
% ciclos. Si es así, ¿por qué no usas más ciclos? ¿No es el objetivo
% de la optimización obtener el mejor de lo mejor? En vez de la sesión
% posterior, teníais que haberos concentrado en analizar el resultado
% anterior, proponer hipótesis sobre los mismos, y si es posible
% mejorarlo. - JJ 

Although this is not the main contribution of this paper, % si no
                                % lo es, ¿por qué lo pones? Estás
                                % introduciendo nuevas posibilidades
                                % para que te ataquen el paper. 
we also show
the resulting behavioural engines of the obtained SurvivalBots,
analysing the distribution of actions and decisions at the beginning
and at the end of the evolution to understand their behaviour. Figure
\ref{figura:tarta_actions} shows the final distribution of the
different types of actions ($attack$, $reinforce$ and $do
nothing$). This figure shows that %As it can be seen, %no digï¿½is 
...
Figure \ref{figura:tarta_attacking} shows the strategy when a planet
is $attacking$ to other. It is clear that the generated bots have a
predilection for nearest planets and the ones easiest to be
conquered. This makes sense because ships flying to destinations far
away are not being used, so, using a $ rush$ strategy makes a good
option to conquer and advance. 
\begin{figure}[htb]
\tiny
\begin{center}
...
Moreover, the generated bots, named SurvivalBots, have been tested
against some state-of-the-art rivals, getting excellent results even
in the comparison with highly evolved and specialized bots. % y les
                                % zurran la badana otros que ya están
                                % publicados también.

% Y no decís nada del análisis del "behavioral engine". Si no concluís
% nada de él, ¿por qué lo ponéis? - JJ
...
% Ni siquiera nuestros propios trabajos están bien citados. Por favor,
% usad la bibliografía canónica en geneura.bib Cooperativebots, por
% ejemplo, está mal (mirad que pone JM Guervós). En la 30 y 32 pone RTS en
% minúsculas. Y así. - JJ 
