...
It is difficult to show the convergence of the populations using a implicit fitness evaluation, as the evaluated individuals (and therefore, the fitness of the average population) do not count with a numeric value to be plotted in time. In this paper, we propose a scoring method that takes into account the number of victories, turns to win and turns to be defeated. After the execution of our algorithm all the generated individuals during all the runs have been confronted versus GPbot and scored using the next formula:
\begin{equation}
Score_{i} = v*2000+t_{win}+t_{defeated}
\end{equation}
where $v$ is the number of victories of the individual against GPbot, $t_{win}$ the total number of turns used to win and $t_{defeated}$ the total number of turns when the individuals has been defeated by GPbot. This score tries to favor the victories against the turns, and...
...
This subsection show the results of the obtained bots. Figure \ref{} shows the distribution of actions and decisions at the beginning of the evolution and at the end. It can be seen than the...
...
This paper presents different implementations of a quite simple approach: to omit the fitness-based selection mechanism in an EA, simulating the evolution in a more natural way: performing real battles of the individuals. This has been applied over the improvement of the behavioural parameters of the bot's AI in the RTS game Planet Wars. This approach has two main benefits: it leaves out existent bots for the evaluation, and the parametrization of the fitness function.
