...
One of the shortcomings of previous works is that they depend on a baseline bot, or a ad-hoc fitness function that involves some kind of parameterization (for example, number of matches or scoring the actions). To deal with this, co-Evolutionary Algorithms (CEAs) have been previously used in this scope, initially regarding puzzle and board games such as Backgammon \cite{Pollack_Backgammon98}, or Go \cite{Runarsson_Go2005}.
...
To avoid this, the optimisation in this paper have been performed using a survival-based evolutionary approach. This can be considered as a co-evolutionary method in which the individuals (bots) fight in game battles in order to decide who survives and moves to the next generation. Two advantages with respect of previous work are tested: not adding ad-hoc parameters (such as the number of victories) and not using a specific bot available for training.
...
This set of parameters have been used previously in the work \cite{GarciaGP14}, obtaining competitive bots. The stop criterion has been set to 4000 evaluation. This is a good number of......
