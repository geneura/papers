...
\item {\em Action}: a leave of the tree (therefore, a ``terminal''). Each decision is the name of the function to execute when the bot is deciding what to do regarding a specific planet (when it is analysing the status of the planet and the possible actions). 
% Antonio - El árbol lo ejecuta un bot para decidir qué hacer considerando un planeta, no lo ejecuta un planeta, ¿no?. Lo he intentado escribir así.
The function indicates to which planet send a percentage of available ships (from 0 to 1), also decided in this step. 
% Antonio - mira a ver si esto es verdad, Fergu. ¿se decide el porcentaje también aquí, ¿no?
\end{itemize}
