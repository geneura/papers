...
\title{MUSES: A corporate user-centric system which applies computational intelligence methods} % Using the present for something that does not exist yet is dangerous - JJ 
...
\maketitle
\begin{abstract}
This work presents the description % == describes - JJ
 of the architecture of a novel enterprise security system, still in
 development, which can % can or will be able? - JJ
 prevent and deal with the security flaws % flaws == defects. Better
                                % isues - JJ
derived from the users in a company. Thus, the Multiplatform Usable
Endpoint Security system (MUSES) considers diverse factors such as the
distribution of information, the type of accesses, the context where
the users are, the category of users or the mix % mix? - JJ
between personal and private data, among others.
This system includes an event correlator and a risk and trust analysis
engine to perform the decision process. MUSES follows a set of defined
security rules, according to the enterprise security policies, but it
is able to self-adapt the decisions and even create new security rules
depending on the user behaviour, the specific device, and the
situation or context. To this aim MUSES applies % intends to apply - JJ
 machine learning and computational intelligence techniques which can
 also be used to predict potential unsafe or dangerous user's
 behaviour. % how do you know this will work? - JJ
\end{abstract}
...
Some years ago, company data were stored in company-owned servers
which were accessed by the employees or users by means of desktop or
portable computers. % This is the same as in Paloma's paper. You have
                    % to qualify this. Who claims that? -JJ
 Nowadays it is frequent that these data are distributed among
 multiple machines, even not all belonging to the company, and being
 consulted and modified through a wide amount of devices, some of them
 owned by the employees.  %reference - JJ
This is the so-called Bring Your Own Device (BYOD) philosophy. % reference - JJ
It is becoming highly successful due to the impact that smartphones
and tablets  are having in the market. % reference
Data security % shouldn't this be "information security"? Also --
              % reference - JJ
and privacy are key factors for a company, thus, to protect them, it
is usual to define Organisational Security Policies. % definition -JJ
Their definition is nowadays a very difficult problem, % sez who?
                                % Reference - JJ
since the BYOD tendency means that several factors must be considered
\cite{Opp_Security11}, most of them previously ignored or
non-considered in the security systems, for instance the current
mixture between personal and professional information in these devices
(the user could surf social networks where there could be friends and
also company partners or clients). % so what? - JJ

Moreover, it has been demonstrated that people are the main hazard
regarding the company security \cite{Adams_Users05}, % this reference
                                % is from before smartphones were
                                % available - JJ
so in this situation, some monitorization and security-aimed
applications are arising. Most of them try to be non-intrusive
(regarding the users' personal data), friendly and easy to manage. % so what? - JJ

This work presents the general architecture of one of these systems,
named MUSES, Multiplatform Usable Endpoint Security system. % which is
                                % better than the state of the art
                                % why? - JJ
It is being implemented by an European Project and will provide a
device independent, user-centric, and self-adaptive corporate security
system, able to cope with the concept of seamless working experience
on different devices. This concept % is it your concept? - JJ
 is a working methodology  which
allows users to start/continue a working session over multiple devices
and locations without any significant loss of data. This new situation
has a big impact from the point of view of the security
\cite{Schu_SecPatterns05}, since the company's data borders have
changed in the last years % data borders? - JJ 
so now the users can access significant data
from outside the enterprise, and possibly through a non absolutely
secure channel. % this should go _before_ you introduce MUSES - JJ
...
The rest of the paper is organised as follows: next section gives a
brief overview on the initial proposal of the MUSES system. Then, the
designed architecture is described in depth in Section
\ref{sec:architecture}. Finally, in Section \ref{sec:advantages} MUSES
features are compared with other existing products, giving an idea on
its advantages, along with the advances beyond the state of the art
that this system aims to. 
...
% Maybe advance beyond the state of the art - JJ
...
There exist in the market some products similar to MUSES, % what's the
                                % name of the product category? - JJ
usually
marketed as Software-as-a-Service (SaaS). The most extended % sez who?
                                % - JJ
 are the IBM's \textit{Hosted Mobile Device Security Management}
 solution \cite{IBM_tool}, 
% Found this paper that cits this solution "Novo, Tatiana
% Silva. "Segurança da informação no uso de smartphones em ambiente
% corporativo." (2013)." Should check it out - JJ
Sophos's \textit{Mobile Control} software solution \cite{Sophos_tool}, 
% This is mentioned also here: http://arxiv.org/abs/1304.0076 Should
% check it out and include in the bibliography  This paper (or thesis,
% or whatever) is also relevant Mylonas, Alexios. "Security and
% Privacy in Ubiquitous Computing: The Smartphone Model and Paradigm."
% - JJ
...
The main differences with MUSES, is that it will be also a free,
open-source, platform independent solution. This is an important
advantage because all the existing tools take into account only
smartphones and tablets, but MUSES covers laptops and company PCs too
(normally servers). Moreover the companies need specific operating
system and server (like Windows Server, for instance). % need them for
                                % what? - JJ
Other big plus of the MUSES system is its new feature of self-adaptiveness. MUSES is able to adapt to changes, either regarding corporate security policies, newly discovered vulnerabilities or threats, different environments of use, or user profiles. 
