...
\item Método de \textit{inferencia de reglas por PG}, por el cual se generarán o crearán nuevas reglas para `cubrir' aquellas situaciones que no están contempladas en el conjunto de reglas inicial. Así, se crearían reglas para poder clasificar aquellos eventos que no pudieron ser clasificados inicialmente. Esta generación de reglas se haría teniendo en cuenta un \textit{diccionario}, es decir, un conjunto de términos correspondiente a todas las posibles entradas y acciones del sistema, las cuales serían respectivamente los antecedentes (condiciones), y las consecuencias a esos antecedentes según las reglas de seguridad. La evaluación de las reglas creadas se realizaría en base a la información registrada en el sistema, de manera que se podría `simular' el comportamiento del sistema incluyendo las nuevas reglas, y se obtendría una medida de si las reglas creadas mejoran o no el rendimiento del sistema.

\item \textit{Refinamiento de reglas por PG}, por el cual se optimizará el conjunto de reglas actual, ajustando los valores de las condiciones (antecedentes). Así, se podría saber qué partes de las reglas serían superfluas o si hay reglas redundantes o que se contradigan entre sí. Se podrían por tanto obtener especializaciones o generalizaciones de las reglas para mejorar el rendimiento del sistema. La evaluación del conjunto total de reglas se haría considerando el número de comportamientos o patrones que se `cubrirían' una vez realizados los ajustes.
